...
/**
 * Cache module-scope della promise di import dinamico di `MapListComponent`.
 ...
 */
let mapListComponentPromise: Promise<any> | null = null;

@Component({
  selector: 'wuic-map-list-lazy',
  standalone: true,
...
export class LazyMapListComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    mapListComponentPromise ??= import('./map-list.component').then((m) => m.MapListComponent);
    this.loadedComponent = await mapListComponentPromise;
