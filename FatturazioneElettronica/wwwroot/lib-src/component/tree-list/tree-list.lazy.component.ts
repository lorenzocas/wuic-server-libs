...
/**
 * Cache module-scope della promise di import dinamico di `TreeListComponent`.
 ...
 */
let treeListComponentPromise: Promise<any> | null = null;

@Component({
  selector: 'wuic-tree-list-lazy',
  standalone: true,
...
export class LazyTreeListComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    treeListComponentPromise ??= import('./tree-list.component').then((m) => m.TreeListComponent);
    this.loadedComponent = await treeListComponentPromise;
