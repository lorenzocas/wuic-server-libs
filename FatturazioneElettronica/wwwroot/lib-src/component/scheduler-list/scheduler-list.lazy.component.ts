...
/**
 * Cache module-scope della promise di import dinamico di `SchedulerListComponent`.
 ...
 */
let schedulerListComponentPromise: Promise<any> | null = null;

@Component({
  selector: 'wuic-scheduler-list-lazy',
  standalone: true,
...
export class LazySchedulerListComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    schedulerListComponentPromise ??= import('./scheduler-list.component').then((m) => m.SchedulerListComponent);
    this.loadedComponent = await schedulerListComponentPromise;
