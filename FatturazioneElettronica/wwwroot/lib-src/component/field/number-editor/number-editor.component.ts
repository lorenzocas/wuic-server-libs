...
@Component({
  selector: 'wuic-number-editor',
  imports: [FormsModule, InputNumberModule, AsyncPipe, SliderModule, NgClass],
  templateUrl: './number-editor.component.html',
  styleUrl: './number-editor.component.scss'
})
export class NumberEditorComponent implements IFieldEditor, AfterViewInit {
  /**
 ...
 */
  constructor() {
    const appSettings = WtoolboxService.appSettings ?? {};
    this.currencyCode = (appSettings as any).currencyCode ?? 'EUR';
    this.locale = (appSettings as any).locale ?? 'it-IT';
  }
   ...
   */
  ngAfterViewInit() {
    if (!this.record || !this.field?.mc_nome_colonna || !this.record[this.field.mc_nome_colonna]) {
      return;
    }
