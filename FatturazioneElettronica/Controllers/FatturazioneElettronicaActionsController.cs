...
public class FatturazioneElettronicaActionsController : ControllerBase
{
    private readonly string _dataConnectionString;
    private readonly ILogger<FatturazioneElettronicaActionsController> _logger;

    public FatturazioneElettronicaActionsController(IConfiguration configuration, ILogger<FatturazioneElettronicaActionsController> logger)
    {
        _logger = logger;
    ...
    public async Task<IActionResult> Execute(string actionKey, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex, "Errore in action {ActionKey}", actionKey);
