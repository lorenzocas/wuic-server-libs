/**
 * Test 62: Dashboard `magazzino_storico` render + screenshot.
 * Format: 4 KPI tile + line chart 30gg + tabella ultimi movimenti.
 */
...
export const meta = {
  id: '62',
  name: 'Dashboard magazzino_storico (4 KPI + line 30gg + tabella movimenti)',
  area: 'dashboard',
  needsUi: true,
  needsApi: false
};
...
export async function run(ctx) {
  ...
  const board = await queryOne(`
    ...
    WHERE boardroute = 'magazzino_storico'
  `);
  assert(board?.boardroute === 'magazzino_storico', 'magazzino_storico non in dom_board');
  ...
  await page.goto(`${baseUrl.replace(/\/$/, '')}/#/magazzino_storico/dashboard?bust=${Date.now()}`, { waitUntil: 'load', timeout: 30000 });
  ...
  assert(counts.charts >= 1, `expected >=1 chart canvas (line 30gg), got ${counts.charts}`);
  assert(counts.listGrid >= 1, `expected >=1 list-grid (movimenti recenti), got ${counts.listGrid}`);

  const snap = `C:\\src\\Wuic\\FatturazioneElettronica\\playwright\\screenshots\\PASS_62_magazzino_storico_${Date.now()}.png`;
  ...
  return { boardroute: 'magazzino_storico', screenshot: snap, size: sz, counts };
}
