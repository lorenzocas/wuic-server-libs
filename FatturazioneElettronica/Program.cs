...
internal static class Program
{
    ...
    private static IHostBuilder CreateHostBuilder(string[] args, string hostProjectRoot, string legacyRoot) =>
        Host.CreateDefaultBuilder(args)            .ConfigureAppConfiguration((hostingContext, config) =>
            ...
            .ConfigureWebHostDefaults(webBuilder =>
            {
                ...
                //   - When the same binary is launched standalone (`dotnet FatturazioneElettronica.dll`),
    ...
    private static string ResolveLegacyRoot(string hostProjectRoot)
    {
        ...
        if (string.IsNullOrWhiteSpace(configured))
        {
            ...
            configured = bootstrapConfig["FatturazioneElettronica:StaticAngularRoot"] ?? bootstrapConfig["NugetHost:StaticAngularRoot"];
        }
