        ...
        private static Func<IDataReader, object> GetDynamicDeserializer(IDataRecord reader, int startBound, int length, bool returnNullIfFirstMissing)
        {
            ...
            return
                 r =>
                 {
                     ...
                     // Oracle: column names returned UPPERCASE by default (case-fold of unquoted
                     // SQL identifiers). Normalize to lowercase here so downstream LINQ matches
                     // (e.g. `x.Key == "foo"`) work cross-dialect. Dict is already
                     // OrdinalIgnoreCase so indexer access (row["X"]) keeps working.
                     bool isOracleReader = r.GetType().Name.IndexOf("Oracle", StringComparison.OrdinalIgnoreCase) >= 0;
                     for (var i = startBound; i < startBound + length; i++)
                     {
                         ...
                         if (isOracleReader && !string.IsNullOrEmpty(colName))
                         {
