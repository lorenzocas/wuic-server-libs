...
#pragma warning restore CS0436 // Type conflicts with imported type
        ...
        private static bool IsTransientError(Exception ex)
        {
            foreach (var current in FlattenExceptions(ex))
            {
                ...
                // MySQL via reflection (no direct type ref to MySql.Data.MySqlClient.MySqlException
                // — Wuic.Webcore non puo' dipendere dal NuGet `MySql.Data` per boundary check
                // scripts/check-plugin-boundaries.ps1, il driver e' confinato in mysql.dll).
                // Stesso pattern del branch Npgsql sotto.
                if (typeName.IndexOf("MySqlException", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    var numberProp = current.GetType().GetProperty("Number");
                    int mySqlNumber = 0;
                    var rawNum = numberProp?.GetValue(current, null);
                    if (rawNum != null) int.TryParse(Convert.ToString(rawNum), out mySqlNumber);
                    switch (mySqlNumber)
                    {
                        case 1042:
                        case 1047:
            ...
            try
            {
                ...
                if (multiExec != null && !(multiExec is string))
                {
                    ...
                    using (var cmd = SetupCommand(cnn, transaction, sql, null, null, commandTimeout, commandType))
                    {
                        ...
                        if (cnn.GetType().Name == "MySqlConnection")
                ...
                string connType = cnn.GetType().Name;

                if (connType == "NpgsqlConnection")
                    identity_scope_query = "SELECT LASTVAL()";
                ...
                if (!string.IsNullOrEmpty(identity_scope_query))
                {
                    using (var cmd = SetupCommand(cnn, transaction, identity_scope_query, null, null, commandTimeout, CommandType.Text))
                    {
                        try
                        {
                            int indxx = sql.ToLower().IndexOf("insert");
                            if (indxx != 0)
                                return totale;
                            ...
                            {
                                int.TryParse(scope_identity.ToString(), out identity_scope);
                            }
                        ...
                        {
                            // PG: `SELECT LASTVAL()` solleva sqlState=55000 ("lastval non e' stato
                            // ancora definito in questa sessione") quando la INSERT precedente NON
                            // ha consumato una sequence (es. md_id esplicito, oppure tabella senza
                            // colonna IDENTITY/SERIAL). E' un esito legittimo per i caller WUIC
                            // che gestiscono l'id lato applicativo: ritorna 0 invece di propagare.
                            // SqlState non e' fortemente tipato (per evitare un reference su Npgsql
                            // qui in Wuic.Webcore), quindi sniffiamo via reflection sul property
                            // `SqlState` (tipo string) tramite duck-typing.
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, string splitOn = "Id", int? commandTimeout = null, CommandType? commandType = null
        ...
        {
            CacheInfo info;
            if (!TryGetQueryCache(identity, out info) || info.Deserializer == null)
            {
                info = new CacheInfo();
        ...
        private static Func<IDataReader, object> GetDynamicDeserializer(IDataRecord reader, int startBound, int length, bool returnNullIfFirstMissing)
        {
            ...
            if (fieldCount <= startBound && fieldCount != 0)
            {
            ...
            return
                 r =>
                 {
                     // PG cross-DBMS parity: case-insensitive comparer so column lookups by alias
                     // work regardless of PG's lowercase-folding of bare identifiers. MSSQL/MySQL/Oracle
                     // returned the alias as-typed; PG returns it lowercased when not double-quoted.
                     IDictionary<string, object> row = new Dictionary<string, object>(length, StringComparer.OrdinalIgnoreCase);
                     // Skip della colonna sentinella `__autogenerated` iniettata da
                     // BuildDynamicSelectQuery cross-DBMS — serve solo a marcare le
                     // query auto-generate lato server (per fixQueryReport e simili)
                     // e NON deve apparire nel result-set client-side
                     // (verificato 2026-05-09: il fix nel `Dapper/SqlMapper.cs` era
                     // inutile perche' Wuic.Webcore.csproj ha `<Compile Remove="Dapper\**"/>`
                     // — il file compilato e' QUESTO `Wuic.Webcore/SqlMapper.cs`).
                     bool firstNonSentinelSeen = false;
                     // Oracle: column names returned UPPERCASE by default per case-fold di
                     // unquoted SQL identifiers. RIMOSSO 2026-05-20 il fix che forzava
                     // ToLowerInvariant: il dict è già OrdinalIgnoreCase quindi LINQ
                     // x.Key == "foo" lavora cross-dialect, ma il forzato lowercase
                     // rompeva la SERIALIZZAZIONE JSON al frontend per template che usano
                     // CamelCase property lookup ({{record.CityName}}). Ora il SQL builder
                     // Oracle emette alias quotati CamelCase (EscapeAliasName), che
                     // Oracle preserva letterali → response key = CamelCase originale.
                     for (var i = startBound; i < startBound + length; i++)
                     {
                         string colName = r.GetName(i);
                         if (string.Equals(colName, "__autogenerated", StringComparison.OrdinalIgnoreCase))
                         {
                             continue;
                         }
                         ...
                         row[colName] = tmp;
                         if (returnNullIfFirstMissing && !firstNonSentinelSeen && tmp == null)
                         {
                             return null;
                         }
                         firstNonSentinelSeen = true;
                     }
...
#pragma warning restore CS0436 // Type conflicts with imported type
        ...
        private static IDbCommand SetupCommand(IDbConnection cnn, IDbTransaction transaction, string sql, Action<IDbCommand, object> paramReader, object obj, int? commandTimeout, CommandType? commandType)
        {
            ...
            // PG fix: Npgsql v8+ rifiuta DateTime con Kind=Local. Le colonne `datetime` MSSQL
            // mappano a PG `timestamp without time zone` (naive). Normalize Kind=Local→Unspecified
            // per matchare la semantica naive. Cross-DBMS safe: per altri provider (SqlClient,
            // MySQL, Oracle) il Kind viene ignorato.
            if (cmd.GetType().Name == "NpgsqlCommand" && cmd.Parameters != null)
            {
                foreach (System.Data.IDbDataParameter p in cmd.Parameters)
                {
                    if (p.Value is DateTime dt && dt.Kind == DateTimeKind.Local)
                    {
