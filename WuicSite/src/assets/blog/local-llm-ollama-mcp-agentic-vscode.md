...
tags: llm, ollama, mcp, cline, continue, local-llm, rag, cost, architecture, ai
---
...
[MCP](https://modelcontextprotocol.io) (Model Context Protocol) is the emerging standard for giving an LLM client *tools*. Cline, Continue, Cursor and Claude Code all speak it. So instead of building a bespoke VS Code extension, we ship a tiny stdio MCP server that wraps the REST API the backend already exposes:
...
It exposes two tools: `wuic_codebase_search` (retrieval — file paths and snippets) and `wuic_ask` (a full RAG answer generated by the local model). It is **zero-dependency** — Node's built-in `http`, no `npm install`, runs on any Node ≥ 14 — and stateless: every call just proxies to the backend. That matters for distribution: it ships inside every WUIC app and is wired up automatically at first run, so an end developer gets a `.mcp.json` already pointing at it.
...
- `.mcp.json` is generated at first run, already pointing at the `wuic-rag` server.
- `.vscode/extensions.json` recommends **Cline** and **Continue** the first time you open the project in VS Code.
- `scripts/mcp/` carries the server, a runbook, and ready-to-paste Cline/Continue configs — plus an `OLLAMA-SETUP.md` that walks you through standing up your own Ollama box (LAN exposure, `num_ctx` tuning, model pull, persistent startup) on Windows or Linux.
