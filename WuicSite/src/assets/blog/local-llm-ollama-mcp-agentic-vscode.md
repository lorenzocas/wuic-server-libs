...
slug: local-llm-ollama-mcp-agentic-vscode
date: 2026-06-14
...
┌──────────────────────────────┐
│ VS Code                      │
│   WUIC Assistant    ───────► │   agentic chat: edit files, run commands
...
[MCP](https://modelcontextprotocol.io) (Model Context Protocol) is the open standard for giving an LLM client *tools*. Cline, Continue, Cursor and Claude Code all speak it. So instead of building a bespoke VS Code extension *at the time*, we shipped a tiny stdio MCP server that wraps the REST API the backend already exposes:

```
You (in the editor):  "where is the cookie login handled?"
   │
   ▼
The agent (local model) decides to call a tool
   │  tools/call  wuic_codebase_search
   ▼
wuic-rag-mcp.mjs  ──POST /api/Rag/Query──►  RAG engine  ──►  top-8 chunks (file:line + snippet)
   │
   ▼
The agent reads the chunks, answers with real citations — or starts editing
```
...
The crucial design point of that first iteration: **the MCP server is not the agent.** It adds *knowledge*, not autonomy. The editing, the command execution, the plan/act loop belonged to the host editor; reusing a mature open-source harness was the entire point. (That is the part that changed — see the update.)
...
- **The WUIC Assistant extension.** Download the `.vsix` from [the model page](https://wuic-framework.com/model) and install it in VS Code (*Extensions ▸ … ▸ Install from VSIX*). It brings its own chat sidebar, agent loop and file edits with diff approval, and defaults to your local Ollama — no API key.
- `.mcp.json` is generated at first run, already pointing at the `wuic-rag` server — any MCP-speaking client picks it up automatically.
- `.clinerules` and `.cursorrules` are generated from the same single source as the agent rules (`AGENTS.md`). WUIC Assistant reads `.clinerules` itself — that is how it inherits the framework conventions and the nudge to search the codebase before inventing. The same files are also why you are not locked into our extension: point Cline or Cursor at the project and they get identical grounding, with the MCP server covering retrieval.
...
You install WUIC Assistant, point it at your Ollama host, and you have a WUIC-aware agentic assistant in your editor for the price of the electricity.
...
**The model is now a Mixture-of-Experts, fine-tuned on our own mistakes.** The dense `qwen2.5-coder:32b` gave way to `qwen3-coder:30b` — a MoE with ~3B active parameters that generates at **~197 tokens/s** on the same 24 GB GPU (the dense 32B did ~30). Then we [DPO-trained it on the assistant's own failure traces](/blog/teaching-a-coding-agent-with-dpo), and that fine-tune is now the default brain for the in-product chatbot too. On the routing suite — grown from 89 to **102 variants** — the base MoE scores 88/102; the DPO fine-tune scores **96/102 at identical speed**. The ~8 s/call in the table above is now ~2–7 s.
...
**The MCP server grew from two tools to nine.** Alongside retrieval and Q&A it now exposes metadata-aware tools — the real column names of a route, lookup resolution (displayed value → ID), sample records, the exact schema of each metadata action, and the framework's skill playbooks — because the single biggest failure mode of a local agent is inventing field names. The chatbot's own tool catalog, meanwhile, is up to **32 tools**.

**The side effect became a product: WUIC Assistant.** Borrowing someone else's agentic harness was the right way to start and the wrong way to stay: the loop we needed is metadata-aware, and grafting that onto a general-purpose agent kept leaking. So the assistant is now **our own VS Code extension**, distributed as a downloadable `.vsix` — chat sidebar, agent loop, file edits with diff approval, `execute_command` in the integrated terminal, and the same `wuic-rag` MCP tools wired in. Three interchangeable brains behind it: local Ollama by default (no key, no bill), the Anthropic API, or an Anthropic subscription through the `claude` CLI. It is on [the model page](https://wuic-framework.com/model).
