...
slug: dpo-first-shot-trajectory-metrics
date: 2026-09-04
...
`qwen3-coder-wuic:30b-dpo` on [Hugging Face](https://huggingface.co/castricolorenzo/qwen3-coder-wuic-30b-dpo) now serves this revision — same tag, same filename, so an `ollama pull` picks it up. GGUF q4_K_M, 18.6 GB, fits a 24 GB card at 48k context. Full setup — Ollama tuning, backend wiring, the VS Code extension — is on the [Run the WUIC coding model locally](/model) page.
