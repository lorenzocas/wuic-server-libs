...
- `rag-llm-provider` — `anthropic` (default) / `openai` / `openrouter` / `ollama`. `ollama` is now a first-class value: it points to a local runtime via `rag-llm-base-url`, with an OpenAI-compatible format.
