...
- `rag-llm-provider` — `anthropic` (default) / `openai` / `openrouter` / `ollama`. `ollama` es ahora un valor de primera clase: apunta a un runtime local vía `rag-llm-base-url`, con formato compatible con OpenAI.
