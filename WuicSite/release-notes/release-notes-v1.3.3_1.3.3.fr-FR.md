...
- `rag-llm-provider` — `anthropic` (défaut) / `openai` / `openrouter` / `ollama`. `ollama` est désormais une valeur de premier ordre : pointe vers un runtime local via `rag-llm-base-url`, au format compatible OpenAI.
