...
![List-grid on a phone — one card per record, action buttons on top, label/value rows for every visible column](https://wuic-framework.com/assets/wuic-framework-docs/screenshots/manual__responsive-mobile__01.png)
...
The public [demo](https://wuic-framework.com/sandbox) runs in any browser at any width. Open DevTools, switch to a phone profile, navigate to any list — you'll see the card swap live. The credentials are on the [sandbox landing page](https://wuic-framework.com/sandbox); login takes 2 seconds.
...
The counterpart to auto-layout on the read side is the drag-and-drop [dashboard designer](https://wuic-framework.com/blog/dashboard-designer-drag-and-drop-metadata) — UI that writes JSON metadata, and how designer-saved boards survive runtime upgrades. And if you're wondering how those routes got scaffolded in the first place, start [here](https://wuic-framework.com/blog/sql-table-to-crud-form-in-30-seconds).
