            ...
            int topRows = 300)
        {
            ...
            try
            {
                ...
                using (metaRawModel mrm = new metaRawModel())
                {
                    ...
                    string BaseCol(string real) => $"{QuoteMySqlIdentifierPivot(baseAlias)}.{QuoteMySqlIdentifierPivot(real)}";
                    string BuildMySqlDateGroupExpr(string sourceExpr, string dateGroupBy)
                    {
                        return (dateGroupBy ?? string.Empty).Trim().ToLowerInvariant() switch
                        {
                            "year" => $"DATE_FORMAT({sourceExpr}, '%Y')",
                            "month" => $"DATE_FORMAT({sourceExpr}, '%Y-%m')",
                            "day" => $"DATE({sourceExpr})",
                            "hour" => $"DATE_FORMAT({sourceExpr}, '%Y-%m-%d %H:00:00')",
                            "minute" => $"DATE_FORMAT({sourceExpr}, '%Y-%m-%d %H:%i:00')",
                            "second" => $"DATE_FORMAT({sourceExpr}, '%Y-%m-%d %H:%i:%s')",
                            _ => sourceExpr
                        };
                    }
                    string ResolveAxisExpr(string alias, Dictionary<string, bool> castDateMap, Dictionary<string, string> groupByMap)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(configuredGroupBy))
                            return BuildMySqlDateGroupExpr(sourceExpr, configuredGroupBy);
                        bool castDate = castDateMap.TryGetValue(alias, out var cast) && cast;
                        return castDate ? $"DATE({sourceExpr})" : sourceExpr;
                    }
                    ...
                    var rowSelects = rowAliases.Select(a =>
                    {
                        ...
                        return $"{ResolveAxisExpr(a, rowCastDateByAlias, rowGroupByByAlias)} AS {QuoteMySqlIdentifierPivot(label)}";
                    }).ToList();

                    var pivotExprs = pivotAliases
                        .Select(a => ResolveAxisExpr(a, colCastDateByAlias, colGroupByByAlias))
                        .Select(expr => $"IFNULL(CAST({expr} AS CHAR), '')")
                        .ToList();
                    string pivotKeyExpr = pivotExprs.Count == 1 ? pivotExprs[0] : $"CONCAT_WS('|', {string.Join(", ", pivotExprs)})";
                    ...
                    if (filterItems != null)
                    {
                        foreach (var f in filterItems.OfType<JObject>())
                        {
                            ...
                            switch (op)
                            {
                                case "eq":
                                case "=":
                                    filterParts.Add((val == null || val.Type == JTokenType.Null) ? $"{colExpr} IS NULL" : $"{colExpr} = {ToMySqlLiteralPivot(val)}");
                                    break;
                                case "neq":
                                case "!=":
                                case "<>":
                                    filterParts.Add((val == null || val.Type == JTokenType.Null) ? $"{colExpr} IS NOT NULL" : $"{colExpr} <> {ToMySqlLiteralPivot(val)}");
                                    break;
                                case "gt":
                                case ">":
                                    filterParts.Add($"{colExpr} > {ToMySqlLiteralPivot(val)}");
                                    break;
                                case "gte":
                                case ">=":
                                    filterParts.Add($"{colExpr} >= {ToMySqlLiteralPivot(val)}");
                    ...
                    string fromTable = (string.IsNullOrWhiteSpace(schemaName) || string.Equals(schemaName, "dbo", StringComparison.OrdinalIgnoreCase))
                        ? $"{QuoteMySqlIdentifierPivot(tableName)} {QuoteMySqlIdentifierPivot(baseAlias)}"
                        : $"{QuoteMySqlIdentifierPivot(schemaName)}.{QuoteMySqlIdentifierPivot(tableName)} {QuoteMySqlIdentifierPivot(baseAlias)}";

                    string distinctPivotSql = $@"
...
ORDER BY __pivot_key";
                    ...
                    using (DbConnection dataConnection = GetOpenConnection(false, connectionName))
                    {
                    ...
                    foreach (var v in valueDefs)
                    {
                        ...
                        foreach (var key in pivotKeys)
                        {
                            ...
                            pivotSelects.Add($"{v.Aggregate}(CASE WHEN {pivotKeyExpr} = '{EscapeSqlStringPivot(key)}' THEN {valueExpr} END) AS {QuoteMySqlIdentifierPivot(outName)}");
                        }
                    }
                    if (!pivotSelects.Any())
                        pivotSelects.Add($"CAST(NULL AS CHAR(1)) AS {QuoteMySqlIdentifierPivot("__empty__")}");
...
{whereClause}{groupByClause}{orderByClause}{limitClause};";
        ...
        private static string QuoteMySqlIdentifierPivot(string identifier)
        {
        ...
        private static string ToMySqlLiteralPivot(JToken token)
        {
...
FOREIGN KEY (`FK_IdChange`) REFERENCES `ChangeMaster`(`IdChange`);");
        ...
        public static string getTableFullName(_Metadati_Tabelle tab)
        {
            ...
            string safetable_name; _ = (string.IsNullOrEmpty(tab.md_db_name) ? "" : "`" + tab.md_db_name + "`.") + (!string.IsNullOrEmpty(tab.md_schema_name) ? "`" + tab.md_schema_name + "`." : ".") + EscapeDBObjectName(table_name);
            if (string.IsNullOrEmpty(tab.md_db_name))
            {
                if (string.IsNullOrEmpty(tab.md_schema_name))
                {
        ...
        public static void clearCustomSettings(string key)
        {
            ...
            using (MySqlConnection connection = metaQueryMySql.GetOpenConnection(true))
            {
                using (metaModelRaw.metaRawModel context = new metaModelRaw.metaRawModel())
                {
                    ...
                    _ = JsonConvert.DeserializeObject(completeSettings);
        ...
        public static rawPagedResult getFlatRecordDistinctComboData(string user_id, string route, string ownerRoute, List<SortInfo> SortInfo, List<GroupInfo> GroupInfo, PageInfo PageInfo, FilterInfos filterInfo, string logicOperator, bool has_server_operation, List<string> columnRestrictionList, string formula_lookup, int mc_id)
        {
            using (metaRawModel context = new metaRawModel())
            {
                _ = context.GetMetadati_Tabelles(ownerRoute).FirstOrDefault();
        ...
        public static string RawUpdateFlatData(Dictionary<string, object> entity, string route, string userId, ref string query, bool isMeta, List<_Metadati_Colonne> metadata, MySqlConnection connection, MySqlTransaction trn)
        {
            ...
            // RecordTranslations sync (mirror MSSQL pipeline). Aggiorna
            // SOLO la lingua runtime dell'utente, e SOLO i campi presenti in
            // entity.__changes. No-op se RecordTranslations non e' abilitato
            // sulla route o l'entity non ha __changes.
            try
            {
                WEB_UI_CRAFTER.RecordTranslationsMySql.OnUpdate(connection, trn, tableMetadata, metadata, entity, userId);
            }
            catch (Exception rtEx)
            {
                RawHelpers.logError(rtEx, "RecordTranslationsMySql.OnUpdate", route);
            }
                ...
                if (collection != null)
                {
                    foreach (Dictionary<string, object> subEntity in collection)
                    {
                        if (subEntity.ContainsKey("___added") && subEntity["___added"] != null && (bool)subEntity["___added"])
                        {
                            ...
                            _ = InsertflatData(newMMEntity, subRoute, userId);
                        }
        ...
        public static string DeleteflatDataByID(int id, string route, string userId)
        {
            ...
            using (MySqlConnection connection = GetOpenConnection(RawHelpers.checkIsMetaData(route), tab.md_conn_name))
            {
                ...
                try
                {
                    ...
                    // RecordTranslations cleanup mirror MSSQL pipeline (no-op
                    // se RT non abilitato sulla route).
                    try
                    {
                        WEB_UI_CRAFTER.RecordTranslationsMySql.OnDelete(connection, null, tab, id.ToString());
                    }
                    catch (Exception rtEx)
                    {
                        RawHelpers.logError(rtEx, "RecordTranslationsMySql.OnDelete", route);
                    }
        ...
        private static string RawDeleteFlatData(Dictionary<string, object> entity, string route, string userId, ref string query, List<_Metadati_Colonne> metadata, bool isMeta, MySqlConnection connection, MySqlTransaction trn)
        {
            ...
            // RecordTranslations cleanup (mirror MSSQL pipeline). Wipe TUTTE
            // le righe (md_id, id_record) cross-language. No-op se RT non
            ...
            string pKeyField = metadata.FirstOrDefault(x => x.mc_is_primary_key)?.mc_nome_colonna;
        ...
        public static string RawInsertFlatData(Dictionary<string, object> entity, string route, string userId, ref string query, List<_Metadati_Colonne> metadata, bool isMeta, MySqlConnection connection, MySqlTransaction trn = null)
        {
            ...
            // RecordTranslations seed (mirror MSSQL pipeline). No-op se la
            ...
            {
                // RecordTranslations e' opt-in via props_bag; un fallimento qui
                // NON deve aborire l'INSERT principale (gia' committato/visible
                // nella stessa transazione). Logghiamo e proseguiamo.
                RawHelpers.logError(rtEx, "RecordTranslationsMySql.OnInsert", route);
            }
                ...
                if (string.IsNullOrEmpty(distinct) && hasServerOperation)
                {
                    ...
                    if (!shadowing)
                    {
                        ...
                        try
                        {
                            ...
                            totalRecords = connection.QueryColumn<long>(countQry, commandTimeout: int.Parse(ConfigHelper.GetSettingAsString("autoGeneratedQueryTimeout"))).FirstOrDefault();
        ...
        public static string BuildDynamicFieldList(metaRawModel mmd, List<_Metadati_Colonne> lst, _Metadati_Tabelle tab, Dictionary<aliasPair, string> joins, string formulaLookup, List<string> joinsAppend, int mcId, _Metadati_Colonne_Lookup lookuprelatedCol, string extraFields = "")
        {
            ...
            if (lst.Count > 0)
            {
                ...
                lst.ForEach((fld) =>
                {
                    ...
                    _ = EscapeDBObjectName(fld.mc_nome_colonna);
                ...
                if (look != null)
                {
                    ...
                    using (metaRawModel ctx = new metaRawModel())
                    {
                        ...
                        if (related != null)
                        {
                            ...
                            string safename; _ = EscapeDBObjectName(look.mc_ui_lookup_entity_name) + "." + EscapeDBObjectName(safeField);
        ...
        private static string FinalizeServerSideGrouping(_Metadati_Tabelle tab, List<_Metadati_Colonne> lst, metaRawModel mmd, List<GroupInfo> GroupInfo, string safetableName, string join, string where, MySqlConnection connection, int skiprecords, PageInfo PageInfo)
        {
            ...
            try
            {
                GroupInfo[0].groupCount = connection.QueryColumn<long>(countGroupQry).FirstOrDefault();
            }
            catch (Exception ex)
            {
        ...
        public static string BuildDynamicUpdateQuery(Dictionary<string, object> entity, List<_Metadati_Colonne> metadata, string user_id, bool importing = false, bool isMeta = false)
        {
            ...
            _ = metadata.OfType<_Metadati_Colonne_Upload>().ToList();

            metadata.Where(x => !x.mc_is_computed.HasValue || !x.mc_is_computed.Value || x.GetType() == typeof(_Metadati_Colonne_Grid)).ToList().ForEach((fld) =>
            {
                if (!entity.ContainsKey(fld.mc_nome_colonna))
                    return;
                    ...
                    if (collection != null)
                    {
                        foreach (Dictionary<string, object> subEntity in collection)
                        {
                            ...
                            if (subEntity.ContainsKey("___added") && (bool)subEntity["___added"])
                            {
                                ...
                                _ = InsertflatData(subEntity, subRoute, user_id);
        ...
        private static bool OptimisticCheck(Dictionary<string, object> entity, string route, List<_Metadati_Colonne> metadata)
        {
            ...
            using (MySqlConnection connection = metaQueryMySql.GetOpenConnection(isMeta, tab.md_conn_name))
            {
                ...
                foreach (string key in original.Keys)
                {
                    ...
                    if (col.mc_db_column_type != "varbinary" && col.mc_db_column_type != "binary" && (!col.mc_is_db_computed.HasValue || !col.mc_is_db_computed.Value) && (!col.mc_is_computed.HasValue || !col.mc_is_computed.Value) && col.mc_db_column_type != "float")
                    {
                        ...
                        {
                            AppendBaseFilter(col, currentFld, "eq", "=", originalValue.ToString(), false, "AND", ref fltr);
                            fltr += ")";
                        }
                    }
                }
                ...
                if (fltr != "")
                {
                    ...
                    try
                    {
                        return connection.QueryColumn<long>(optQry).FirstOrDefault() > 0;
                    }
                    catch (Exception ex)
                    {
            ...
            _Metadati_Colonne_Grid manyToManyColumn)
        {
            ...
            string safeMmTable = RawHelpers.getStoreTableName(mmTable, "mysql");
            string localField = RawHelpers.escapeDBObjectName(manyToManyColumn.mc_ui_grid_manytomany_local_id_field, "mysql");
            string relatedField = RawHelpers.escapeDBObjectName(manyToManyColumn.mc_ui_grid_manytomany_related_id_field, "mysql");
            string safeLocalValue = EscapeValueStrict(localId)?.ToString() ?? "null";
            ...
            {
                using (MySqlConnection mmConnection = GetOpenConnection(false, mmConnName))
                {
        ...
        public static string BuildDynamicInsertQuery(IDictionary<string, object> entity, List<_Metadati_Colonne> metadata, string user_id, out string generated_pkey, bool importing = false, bool parametric = false)
        {
            ...
            _ = metadata.OfType<_Metadati_Colonne_Upload>().Where(x => !string.IsNullOrEmpty(x.MultipleUploadBlobFieldName)).Select(x => x.MultipleUploadBlobFieldName).ToList();

            metadata.Where(x => !x.mc_is_computed.HasValue || !x.mc_is_computed.Value).ToList().ForEach((fld) =>
            {
        ...
        public static void ManageShadowCaching(dynamic caching, MySqlConnection con, metaRawModel mmd, _Metadati_Tabelle tab, List<_Metadati_Colonne> lst, string safetableName, string fieldList, string join, string where, string orderBy, int skiprecords, PageInfo PageInfo, List<SortInfo> SortInfo, FilterInfos filterInfo, List<AggregationInfo> aggregates, List<AggregationResult> aggregateValues, string logicOperator, string distinct, Dictionary<aliasPair, string> joins, string formulaLookup, string userId, int mcId, _Metadati_Colonne linkedCol, ref string finalQry, out long totalRecords)
        {
            ...
            if (string.IsNullOrEmpty(distinct))
            {
                ...
                try
                {
                    ...
                    totalRecords = con.QueryColumn<long>(countQry, commandTimeout: int.Parse(ConfigHelper.GetSettingAsString("autoGeneratedQueryTimeout"))).FirstOrDefault();
        ...
        public static decimal readProgress(string guid)
        {
            using (DbConnection connection = GetOpenConnection(true))
            {
                ...
                return connection.QueryColumn<decimal>("SELECT coalesce(progress, -1) FROM _progress_indicator WHERE guid=@guid", dbArgs).FirstOrDefault();
            }
        }
        ...
        public static void fixQueryReport(string user_id, dynamic report, string route, DbConnection connection, ref int needFilter, string[] filterSplit)
        {
            ...
            if (lst.Count > 0)
            {
                ...
                if (!tbl.md_is_stored)
                {
                    ...
                    {
                        // Riscrivi SOLO se vuoto o gia' auto-generato (sentinella
                        // `__autogenerated` iniettata da BuildDynamicSelectQuery).
                        // Vedi `metaQuery.fixQueryReport` (MSSQL) per il razionale
                        // del preserve-custom-SQL: comportamento identico cross-DBMS.
                        var prop = ds.GetType().GetProperty("SqlCommand");
                        if (prop != null && prop.CanWrite)
                        {
                            string existingSql = prop.GetValue(ds) as string ?? string.Empty;
                            bool isAutogenOrEmpty =
                                string.IsNullOrWhiteSpace(existingSql) ||
                                existingSql.IndexOf(metaQuery.AutogeneratedSentinelToken, StringComparison.OrdinalIgnoreCase) >= 0;
                            if (isAutogenOrEmpty)
