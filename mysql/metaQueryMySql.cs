...
namespace metaModelRaw
{
    public partial class metaQueryMySql
    {
        ...
        // nomi parametro, nomi setting, valori metadata e format string .NET.
        // ------------------------------------------------------------------

        // Chiavi JSON delle opzioni pivot (rowColumnOptions/columnColumnOptions).
        ...
        // Chiavi JSON/dizionario dei filtri e dei change-log.
        private const string FieldKey = "field";
        private const string ValueKey = "value";
        private const string OldValueKey = "oldValue";
        private const string TimestampKey = "timestamp";

        // Token degli operatori di filtro (identificatori interni, non SQL).
        ...
        private const string OpEqAll = "eqall";

        // Discriminatore provider per gli helper cross-dbms (escape/quote/formati).
        private const string MySqlProviderName = "mysql";

        // Nomi di connection string / setting in appsettings.
        private const string MetaConnectionName = "MetaDataSQLConnection";
        private const string AutoQueryTimeoutSetting = "autoGeneratedQueryTimeout";
        private const string LogicDeleteFieldSetting = "logicDeleteField";
        private const string FalseString = "false";

        // Valori di mc_ui_column_type / mc_db_column_type (metadata, non SQL).
        private const string ColTypeNumber = "number";
        private const string ColTypeBoolean = "boolean";
        private const string ColTypeLookupById = "lookupByID";
        private const string ColTypeMultipleCheck = "multiple_check";
        private const string ColTypeDatetime = "datetime";
        private const string ColTypeNumberBoolean = "number_boolean";
        private const string ColTypeNumberSlider = "number_slider";
        private const string ColTypeUpload = "upload";
        private const string ColTypeHtmlArea = "html_area";
        private const string ColTypeVarchar = "varchar";
        private const string ColTypePoint = "point";
        private const string ColTypeGeometry = "geometry";

        // Nome fisico della tabella reticolare (valore, poi escapato dagli helper).
        private const string ReticularTableName = "tabella_reticolare";
        // Template del nome delle colonne testo generate sulle route reticolari.
        private const string ReticularTextColumnNameFormat = "colonna_{0}_testo";

        // Nomi colonna/parametro di _metadati__colonne (MySqlParameter + chiavi entity).
        private const string MetaColVoaClass = "voa_class";
        private const string MetaColMdId = "md_id";
        private const string MetaColDbColumnType = "mc_db_column_type";
        private const string MetaColDisplayStringInEdit = "mc_display_string_in_edit";
        private const string MetaColDisplayStringInView = "mc_display_string_in_view";
        private const string MetaColLogicEditable = "mc_logic_editable";
        private const string MetaColLogicNullable = "mc_logic_nullable";
        private const string MetaColNomeColonna = "mc_nome_colonna";
        private const string MetaColUiColumnType = "mc_ui_column_type";
        private const string MetaColComputedFormula = "mccomputedformula";
        private const string MetaColIsComputed = "mciscomputed";
        private const string MetaColGrantByDefault = "mcgrantbydefault";
        private const string MetaColOrdine = "mcordine";

        // Chiavi "di servizio" dell'entity/record lato client.
        private const string SelectedRowKey = "___selected";
        private const string AddedRowKey = "___added";
        private const string DeletedRowKey = "___deleted";
        private const string GuidEntityKey = "__guid";
        private const string ChangesEntityKey = "__changes";
        private const string OriginalEntityKey = "__original";
        private const string ExtraFilterFieldName = "__extra";
        private const string CountOutParamName = "count__";

        // Nomi metodo per customizeException/traceQuery e marker di log errore.
        private const string DeleteFlatDataMethodName = "DeleteflatData";
        private const string RestoreFlatDataMethodName = "RestoreflatData";
        private const string ExecutedQueryMarker = "****EXECUTED QUERY:****";

        // Format string .NET per date (il valore formattato finisce poi nella query).
        private const string SqlDateTimeFormat = "yyyy-MM-dd HH:mm:ss";
        private const string CompactDateFormat = "yyyyMMdd";
        private const string CompactDateTimeFormat = "yyyyMMdd HH:mm:ss";
            ...
            try
            {
                ...
                using (metaRawModel mrm = new metaRawModel())
                {
                    ...
                    var valueDefs = effectiveValueDefs
                        ...
                        {
                            string caption;
                            if (string.IsNullOrWhiteSpace(v.Caption))
...
{whereClause}{groupByClause}{orderByClause}{limitClause};";
                    ...
                    response["dbms"] = MySqlProviderName;
...
FOREIGN KEY (`FK_IdChange`) REFERENCES `ChangeMaster`(`IdChange`);");
                    ...
                    {
                        // Best-effort: la FK e' opzionale (dati legacy possono violarla);
                        // il change-tracking funziona anche senza il vincolo.
                    }
                }
        ...
        public static MySqlConnection GetOpenConnection(bool isMetaDataQuery, string connectionName = "", user u = null)
        {
            ...
            if (string.IsNullOrEmpty(connectionString))
            {
                ...
                {
                    bool connectionByUser = bool.Parse(ConfigHelper.GetSettingAsString("connectionByUser") ?? FalseString);
            ...
            {
                throw new InvalidOperationException("Connection string not defined. Please install first!");
            }
        }
        ...
        public static int ExecuteMySqlScriptWithCount(DbConnection connection, string script, Action<long> onProgress)
        {
            ...
            try
            {
                ...
                try { onProgress?.Invoke(script.Length); } catch { /* progress best-effort: un errore della callback non deve far fallire lo script */ }
                return 1;
            }
            finally
            {
        ...
        public static void FlushCache(string route)
        {
            ...
            bool isSys = false;
            try { isSys = RawHelpers.checkIsMetaData(route); } catch { /* best-effort: route sconosciuta => trattata come non-system, il flush prosegue */ }
            if (isSys) return;

            string shadowTableName = RawHelpers.escapeDBObjectName("_shadow_" + route, MySqlProviderName);
        ...
        {
            using (metaModelRaw.metaRawModel mmd = new metaModelRaw.metaRawModel(MySqlProviderName))
            {
                ...
                if (tab != null)
                {
                    ...
                    int text_col_count = cols.Count(x => x.mc_db_column_type == ColTypeVarchar);
                    int numeric_col_count = cols.Count(x => x.mc_db_column_type == "decimal" || x.mc_db_column_type == "bit");
                    int total_col_count = cols.Count;

                    if (type == "1")
                    {
                        col_name = string.Format(ReticularTextColumnNameFormat, (text_col_count + 1).ToString().PadLeft(3, '0'));
                        db_col_type = ColTypeVarchar;
                        ...
                        using (MySqlConnection con = GetOpenConnection(true))
                        {
                            ...
                            cmd.Parameters.Add(new MySqlParameter(MetaColVoaClass, 1));
                            cmd.Parameters.Add(new MySqlParameter(MetaColMdId, tab.md_id));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDbColumnType, db_col_type));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInEdit, col_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInView, col_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColLogicEditable, true));
                            cmd.Parameters.Add(new MySqlParameter(MetaColLogicNullable, true));
                            cmd.Parameters.Add(new MySqlParameter(MetaColNomeColonna, col_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColUiColumnType, mc_ui_column_type));
                            cmd.Parameters.Add(new MySqlParameter(MetaColComputedFormula, !isReticular ? "null" : ""));
                            cmd.Parameters.Add(new MySqlParameter(MetaColIsComputed, !isReticular));
                            cmd.Parameters.Add(new MySqlParameter(MetaColGrantByDefault, true));
                            cmd.Parameters.Add(new MySqlParameter(MetaColOrdine, total_col_count));
                    ...
                    else if (type == "3")
                    {
                        ...
                        db_col_type = "decimal";
                        mc_ui_column_type = ColTypeNumber;

                        string query = "INSERT INTO `_metadati__colonne` (`voa_class`, `md_id`, `mc_db_column_type`, `mc_display_string_in_edit`, `mc_display_string_in_view`, `mc_logic_editable`, `mc_logic_nullable`, `mc_nome_colonna`, `mc_ui_column_type`, `mccomputedformula`, `mciscomputed`, `mcgrantbydefault`, `mcordine`) VALUES (@voa_class, @md_id, @mc_db_column_type, @mc_display_string_in_edit, @mc_display_string_in_view, @mc_logic_editable, @mc_logic_nullable, @mc_nome_colonna, @mc_ui_column_type, @mccomputedformula, @mciscomputed, @mcgrantbydefault, @mcordine); SELECT LAST_INSERT_ID()";
                        using (MySqlConnection con = GetOpenConnection(true))
                        {
                            MySqlCommand cmd = new MySqlCommand(query, con);
                            cmd.Parameters.Add(new MySqlParameter(MetaColVoaClass, 3));
                            cmd.Parameters.Add(new MySqlParameter(MetaColMdId, tab.md_id));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDbColumnType, db_col_type));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInEdit, col_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInView, col_name));
                    ...
                    else if (type == "4")
                    {
                        col_name = string.Format("colonna_{0}_numero", (numeric_col_count + 1).ToString().PadLeft(3, '0'));
                        db_col_type = "bit";
                        mc_ui_column_type = ColTypeBoolean;
                        ...
                        using (MySqlConnection con = GetOpenConnection(true))
                        {
                            ...
                            cmd.Parameters.Add(new MySqlParameter(MetaColMdId, tab.md_id));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDbColumnType, db_col_type));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInEdit, display_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInView, display_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColLogicEditable, true));
                            cmd.Parameters.Add(new MySqlParameter(MetaColLogicNullable, true));
                            cmd.Parameters.Add(new MySqlParameter(MetaColNomeColonna, col_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColUiColumnType, mc_ui_column_type));
                    ...
                    else if (type == "5")
                    {
                        col_name = string.Format("colonna_{0}_numero", (numeric_col_count + 1).ToString().PadLeft(3, '0'));
                        db_col_type = "int";
                        mc_ui_column_type = ColTypeLookupById;
                        ...
                        {
                            MySqlCommand cmd = new MySqlCommand(query, con);
                            cmd.Parameters.Add(new MySqlParameter(MetaColVoaClass, 2));
                            cmd.Parameters.Add(new MySqlParameter(MetaColMdId, tab.md_id));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDbColumnType, db_col_type));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInEdit, display_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInView, display_name));
                    ...
                    else if (type == "6")
                    {
                        ...
                        {
                            MySqlCommand cmd = new MySqlCommand(query, con);
                            cmd.Parameters.Add(new MySqlParameter(MetaColVoaClass, 6));
                            cmd.Parameters.Add(new MySqlParameter(MetaColMdId, tab.md_id));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDbColumnType, db_col_type));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInEdit, display_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInView, display_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColLogicEditable, false));
                            cmd.Parameters.Add(new MySqlParameter(MetaColLogicNullable, true));
                            cmd.Parameters.Add(new MySqlParameter(MetaColNomeColonna, col_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColUiColumnType, mc_ui_column_type));
                            cmd.Parameters.Add(new MySqlParameter(MetaColComputedFormula, !isReticular ? "''" : ""));
                            cmd.Parameters.Add(new MySqlParameter(MetaColIsComputed, !isReticular));
                            cmd.Parameters.Add(new MySqlParameter(MetaColGrantByDefault, true));
                            cmd.Parameters.Add(new MySqlParameter(MetaColOrdine, total_col_count));
                    ...
                    else if (type == "7")
                    {
                        col_name = string.Format(ReticularTextColumnNameFormat, (text_col_count + 1).ToString().PadLeft(3, '0'));
                        db_col_type = ColTypeVarchar;
                        mc_ui_column_type = ColTypeMultipleCheck;
                        ...
                        {
                            MySqlCommand cmd = new MySqlCommand(query, con);
                            cmd.Parameters.Add(new MySqlParameter(MetaColVoaClass, 4));
                            cmd.Parameters.Add(new MySqlParameter(MetaColMdId, tab.md_id));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDbColumnType, db_col_type));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInEdit, display_name));
                            cmd.Parameters.Add(new MySqlParameter(MetaColDisplayStringInView, display_name));
                            ...
                            cmd.Parameters.Add(new MySqlParameter(MetaColGrantByDefault, true));
                            cmd.Parameters.Add(new MySqlParameter(MetaColOrdine, total_col_count));
                            cmd.Parameters.Add(new MySqlParameter(MetaColComputedFormula, "''"));
                            cmd.Parameters.Add(new MySqlParameter(MetaColIsComputed, true));
        ...
        public static void logOut(user user)
        {
            ...
            if (HttpContext.Current != null && HttpContext.Current.Request.Cookies["user"] != null)
            {
        ...
        public static user login(string user_name, string password, SysInfo infos)
        {
            using (MySqlConnection connection = string.IsNullOrEmpty(infos.user_db_name) ? GetOpenConnection(true) : getSpecificConnection(infos.user_db_name))
            {
                bool isPwdEncripted = bool.Parse(ConfigHelper.GetSettingAsString("IsPwdEncripted") ?? FalseString);
        ...
        public static List<role> getMultipleRoleRoleByUserID(string user_id)
        {
            using (metaRawModel context = new metaRawModel())
            {
                ...
                using (MySqlConnection connection = string.IsNullOrEmpty(infos.role_db_name) ? GetOpenConnection(true) : getSpecificConnection(infos.role_db_name))
                {
                    ...
                    string query = string.Format("SELECT {0}.{1}, {0}.{2} FROM {0} inner join utenti_ruoli on utenti_ruoli.{1} = {0}.{1} inner join utenti on utenti_ruoli.{5} = {4}.{5} WHERE {4}.{3}=@uid", infos.role_table_name, infos.role_id_column_name, infos.role_description_column_name, infos.user_id_column_name, infos.user_table_name, infos.user_id_column_name);
        ...
        public static List<Dictionary<string, object>> convertDataReaderToDictionaryList(MySqlDataReader dr)
        {
            ...
            while (dr.Read())
            {
                ...
                for (int i = 0; i < dr.FieldCount; i++)
                {
                    ...
                    {
                        throw new InvalidOperationException("Chiave {0} già aggiunta. Verificare che la query non torni campi con lo stesso alias.");
                    }
        ...
        private static string GetTableName(_Metadati_Tabelle tab)
        {
            ...
            if (!string.IsNullOrEmpty(tab.md_db_name))
                tablename = RawHelpers.escapeDBObjectName(tab.md_db_name, MySqlProviderName) + ".";
            else if (tab.is_system_route)
            {
                string cs = ConfigHelper.ResolveConnectionString(MetaConnectionName);
                var csb = new MySqlConnectionStringBuilder(cs);
                string metadataDbName = csb.Database;
                tablename = RawHelpers.escapeDBObjectName(metadataDbName, MySqlProviderName) + ".";
            }

            if (!string.IsNullOrEmpty(tab.md_schema_name))
                tablename += RawHelpers.escapeDBObjectName(tab.md_schema_name, MySqlProviderName) + ".";
            else if (!string.IsNullOrEmpty(tab.md_db_name))
                tablename += "";

            tablename += RawHelpers.escapeDBObjectName(tab.md_nome_tabella, MySqlProviderName);
                                            ...
                                            if (!string.IsNullOrEmpty(adminSettings))
                                            {
                                                using (StringReader sra = new StringReader(adminSettings))
                                                {
                                                    using (JsonTextReader readera = new JsonTextReader(sra))
                                                    {
                                                        ...
                                                        foreach (KeyValuePair<string, object> prop in adminData.GetProperties().Where(p => !data.Contains(p)))
                                                        {
                ...
                for (int idx = 0; idx < dataObject.Count; idx++)
                {
                    for (int mdx = 0; mdx < modelObject.Count; mdx++)
                    {
                        ...
                        if (objVal != null && objVal.GetType() == typeof(JArray))
                        {
                            ...
                            {
                                // Hook host ha gia' scritto la cella (true = gestito):
                                // nessuna ulteriore elaborazione qui.
                            }
                            else
                            {
                                ...
                                if (!string.IsNullOrEmpty(gridCol.mc_ui_grid_display_field))
                                {
                                    foreach (JObject value in values)
                                    {
                                        ...
                                        bool selected = value[SelectedRowKey].ToObject<bool>();

                                        if (selected)
                                            display.Append((display.Length == 0 ? "" : ", ") + valo);
                                    }
                                }
                                else
                                {
                                    foreach (JObject value in values)
                                    {
                                        ...
                                        {
                                            display.Append((display.Length == 0 ? "" : ", ") + string.Format("{0}: {1}", x.Key, x.Value));
                                        }
                                    }
                                }

                                Excel.SetCellValue(spreadsheet, worksheet, Convert.ToUInt32(mdx + 1), Convert.ToUInt32(idx + 2),
                                    RawHelpers.ParseNull(display.ToString()), false, false);
                            }
                        }
                        else
                        {
                            ...
                            if (val != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    else if (type == ColTypeNumberBoolean)
                                    {
                                        if (RawHelpers.ParseNull(val.Value) == string.Empty)
                                        {
                                            Excel.SetCellValue(spreadsheet, worksheet, Convert.ToUInt32(mdx + 1), Convert.ToUInt32(idx + 2), "", false, false);
                                    ...
                                    else if (type == ColTypeLookupById)
                                    {
            ...
            using (metaRawModel context = new metaRawModel())
            {
                ...
                if (tabel != null)
                {
                    ...
                    using (DbConnection connection = GetOpenConnection(isMeta, tabel.md_conn_name))
                    {
                        ...
                        try
                        {
                            List<Dapper.SqlMapper.FastExpando> rows = (List<Dapper.SqlMapper.FastExpando>)connection.Query(query, commandTimeout: int.Parse(ConfigHelper.GetSettingAsString(AutoQueryTimeoutSetting)));
                            return rows.Count == 0;
                        }
                        catch (MySqlException ex1)
                        {
                            if (ex1.Number != 421)
                                throw new InvalidOperationException(ex1.Message + ExecutedQueryMarker + query);
                            else
                            {
                                return false;
                            }
                        ...
                        {
                            throw new InvalidOperationException(EX.Message + ExecutedQueryMarker + query);
                        }
        ...
        public static rawPagedResult GetFlatData(string user_id, string route, int lookup_table_id = 0, List<SortInfo> SortInfo = null, List<GroupInfo> GroupInfo = null, PageInfo PageInfo = null, FilterInfos filterInfo = null, string logicOperator = "AND", bool has_server_operation = true, List<AggregationInfo> aggregates = null, List<string> columnRestrictionLists = null, string formula_lookup = "", int mc_id = 0, bool skipNested = false, string extraFields = "", SerializableDictionary<string, object> currentRecord = null)
        {
            ...
            if (!string.IsNullOrEmpty(formula_lookup))
            {
                ...
                if (otherCol != null)
                    redux.Add(new _Metadati_Colonne() { mc_nome_colonna = otherCol.mc_nome_colonna, _Metadati_Tabelle = lst[0]._Metadati_Tabelle, mc_computed_formula = formula_lookup, mc_ui_column_type = "text", mc_is_computed = true, mc_grant_by_default = true });
            ...
            if (lst.Count > 0)
            {
                ...
                using (MySqlConnection connection = GetOpenConnection(RawHelpers.checkIsMetaData(route), tab.md_conn_name))
                {
                    try
                    {
                        ...
                        if (rows == null)
                        {
                            rows = (List<Dapper.SqlMapper.FastExpando>)connection.Query(query, commandTimeout: int.Parse(ConfigHelper.GetSettingAsString(AutoQueryTimeoutSetting)));
        ...
        public static rawPagedResult getFlatRecordDistinctComboData(string user_id, string route, string ownerRoute, List<SortInfo> SortInfo, List<GroupInfo> GroupInfo, PageInfo PageInfo, FilterInfos filterInfo, string logicOperator, bool has_server_operation, List<string> columnRestrictionList, string formula_lookup, int mc_id)
        {
            using (metaRawModel context = new metaRawModel())
            {
                ...
                if (col != null)
                {
                    ...
                    List<Dapper.SqlMapper.FastExpando> results = res.results.OfType<Dapper.SqlMapper.FastExpando>().Where(z => z.data[lookupAlias] != null).GroupBy(x => x.data[col.mc_nome_colonna].ToString()).Select(x => x.First()).OrderBy(z => z.data[lookupAlias].ToString()).ToList();

                    foreach (var rowData in results.Select(x => x.data))
                    {
                        if (fi != null && !string.IsNullOrEmpty(fi.value))
                        {
                            if (fi != null && fi.operatore == OpStartsWith)
                            {
                                if (!rowData[lookupAlias].ToString().ToLower().StartsWith(fi.value.ToLower()))
                                    continue;
                            }
                            else if (fi != null && fi.operatore == OpContains)
                            {
                                if (!rowData[lookupAlias].ToString().ToLower().Contains(fi.value.ToLower()))
                                    continue;
                            }
                            else if (fi != null && fi.operatore == OpEndsWith && !rowData[lookupAlias].ToString().ToLower().EndsWith(fi.value.ToLower()))
                            {
                                continue;
                            }
                        }
                        ...
                        object idValue = rowData[col.mc_nome_colonna];
                        ...
                        {
                            cmbItem.Add(col.mc_ui_lookup_dataValueField, rowData[col.mc_nome_colonna]);
                            cmbItem.Add(col.mc_ui_lookup_dataTextField, rowData[lookupAlias]);
        ...
        public static string GetValueFromStored(string user_id, string stored, List<filterElement> parameters)
        {
            using (metaRawModel context = new metaRawModel())
            {
                ...
                if (metaStored != null)
                {
                    ...
                    {
                        stored = RawHelpers.getStorePrefix(metaStored, MySqlProviderName) + RawHelpers.getDBEntityQuoteSymbol(MySqlProviderName) + metaStored.md_nome_tabella + RawHelpers.getDBEntityQuoteSymbol(MySqlProviderName, false);
        ...
        public static DataTable GetSchemaFromStored(string stored, List<filterElement> parameters)
        {
            using (metaRawModel context = new metaRawModel())
            {
                ...
                if (metaStored != null)
                {
                    ...
                    {
                        stored = RawHelpers.getStoreTableName(metaStored, MySqlProviderName);
            ...
            using (metaRawModel context = new metaRawModel())
            {
                ...
                if (metaStored != null)
                {
                    using (MySqlConnection connection = GetOpenConnection(false, metaStored.md_conn_name))
                    {
                        ...
                        if (parameterDefinition != null)
                        {
                            foreach (JToken jt in parameterDefinition)
                            {
                                ...
                                if (pair != null)
                                {
                                    ...
                                    else if (pair.field == CountOutParamName)
                        ...
                        if (dbArgs.ParameterNames.Any(x => x == CountOutParamName))
                            conto = long.Parse(dbArgs.Get<string>(CountOutParamName));

                        foreach (var outField in parameters.Where(x => x.isOut).Select(pair => pair.field))
                        {
                            if (outField != CountOutParamName)
                            {
                                if (rows.Count == 0)
        ...
        public static string RawUpdateFlatData(Dictionary<string, object> entity, string route, string userId, ref string query, bool isMeta, List<_Metadati_Colonne> metadata, MySqlConnection connection, MySqlTransaction trn)
        {
            ...
            string result = connection.Execute(query, commandTimeout: int.Parse(ConfigHelper.GetSettingAsString(AutoQueryTimeoutSetting)), transaction: trn).ToString();
            watch.Stop();
            ...
            multiple_check_fixes.ForEach(colGrid =>
            {
                ...
                if (collection != null)
                {
                    ...
                    {
                        if (subEntity.ContainsKey(AddedRowKey) && subEntity[AddedRowKey] != null && (bool)subEntity[AddedRowKey])
                        {
                            if (subEntity.ContainsKey(DeletedRowKey))
                            {
                                object deleted = subEntity[DeletedRowKey];
                        ...
                        else if (subEntity.ContainsKey(DeletedRowKey) && (bool)subEntity[DeletedRowKey] && (bool)subEntity[SelectedRowKey])
                        {
            ...
            upload_fixes.ForEach(upload_fix =>
            {
                ...
                if (entity.ContainsKey(GuidEntityKey))
                    __id_src = entity[GuidEntityKey]?.ToString();
            ...
            {
                RawHelpers.InvokeUtilityHook(metadata[0]._Metadati_Tabelle.md_after_save_server_method_name, new object[] { userId, entity, dataMode.edit, "" });
            }
            ...
            if (enableServerSideCrudChangeLog && entity.ContainsKey(ChangesEntityKey))
            {
                List<Dictionary<string, object>> changess = GetDictionaryListValue(entity, ChangesEntityKey);
                ...
                changess.ForEach(x =>
                {
                    ...
                    ct.field = x[FieldKey].ToString();
                    ct.oldValue = RawHelpers.ParseNull(x[OldValueKey]);
                    ct.newValue = RawHelpers.ParseNull(x["newValue"]);
                    ct.timestamp = DateTime.Parse(x[TimestampKey].ToString());
                    ...
                    using (MySqlConnection connection2 = GetOpenConnection(false))
                    {
                        try
                        {
                            ...
                            foreach (ChangeT change in changes)
                            {
                                ...
                                dbArgs.Add(FieldKey, metaQueryMySql.EscapeValue(change.field));
        ...
        public static string DeleteflatData(Dictionary<string, object> entity, string route, string userId, MySqlConnection conn = null, MySqlTransaction trn = null)
        {
            ...
            {
                List<_Metadati_Colonne> metadata = _Metadati_Colonne.getColonneByUserID(route, 0, userId, dataMode.insert, null);
                tab = metadata[0]._Metadati_Tabelle;
                bool isMeta = RawHelpers.checkIsMetaData(route);
            ...
            {
                Exception customEx = Utility.customizeException(DeleteFlatDataMethodName, e1, user.getUserByID(userId), route, query, entity);
                throw customEx;
            }
            catch (MySqlException e2)
            {
                if (e2.Number == 1451 || e2.Number == 1452 || e2.Number == 547)
                {
                    ...
                    Exception customEx = Utility.customizeException(DeleteFlatDataMethodName, ve, user.getUserByID(userId), route, query, entity);
                    throw customEx;
                }
                else
                {
                    Exception customEx = Utility.customizeException(DeleteFlatDataMethodName, e2, user.getUserByID(userId), route, query, entity);
            ...
            {
                Exception customEx = Utility.customizeException(DeleteFlatDataMethodName, e3, user.getUserByID(userId), route, query, entity);
                RawHelpers.logError(customEx, "deleteFlatData", query);
                throw customEx;
            }
        }
        ...
        private static string RawDeleteFlatData(Dictionary<string, object> entity, string route, string userId, ref string query, List<_Metadati_Colonne> metadata, bool isMeta, MySqlConnection connection, MySqlTransaction trn)
        {
            ...
            var watch = Stopwatch.StartNew();
            SetLastCrudSqlQuery(query);
            string ret = connection.Execute(query, commandTimeout: int.Parse(ConfigHelper.GetSettingAsString(AutoQueryTimeoutSetting)), transaction: trn).ToString();
            watch.Stop();
            RawHelpers.traceQuery(DeleteFlatDataMethodName, query, watch.ElapsedMilliseconds, route);
            ...
            if (!string.IsNullOrEmpty(metadata[0]._Metadati_Tabelle.md_after_save_server_method_name))
            {
                RawHelpers.InvokeUtilityHook(metadata[0]._Metadati_Tabelle.md_after_save_server_method_name, new object[] { userId, entity, dataMode.delete, "" });
            }
            ...
            foreach (var kv in entity)
            {
                ...
                {
                    { FieldKey, kv.Key },
                    { OldValueKey, RawHelpers.ParseNull(kv.Value) },
                    { TimestampKey, DateTime.Now }
                });
            }
            ...
            if (deleteChanges.Count == 0 && !string.IsNullOrEmpty(pKeyField))
            {
                ...
                {
                    { FieldKey, pKeyField },
                    { OldValueKey, RawHelpers.ParseNull(pKeyValue) },
                    { TimestampKey, DateTime.Now }
                });
            }
            ...
            if (enableServerSideCrudChangeLog && deleteChanges.Count > 0)
            {
                ...
                string globalLogicDeleteField = RawHelpers.ParseNull(ConfigHelper.GetSettingAsString(LogicDeleteFieldSetting));
                ...
                else
                {
                    ...
                    operationType = metadata[0]._Metadati_Tabelle.md_has_logic_delete && hasLogicalDeleteKey
                        ? "delete_logical"
                        : "delete_physical";
                }
                ...
                using (MySqlConnection connection2 = GetOpenConnection(false))
                {
                    try
                    {
                        ...
                        foreach (var change in deleteChanges)
                        {
                            var dbArgs = new DynamicParameters();
                            dbArgs.Add("masterId", metaQueryMySql.EscapeValue(masterId));
                            dbArgs.Add(FieldKey, metaQueryMySql.EscapeValue(change[FieldKey]));
        ...
        public static string RawInsertFlatData(Dictionary<string, object> entity, string route, string userId, ref string query, List<_Metadati_Colonne> metadata, bool isMeta, MySqlConnection connection, MySqlTransaction trn = null)
        {
            ...
            multiple_check_fixes.ForEach(colGrid =>
            {
                ...
                if (collection != null)
                {
                    foreach (Dictionary<string, object> subEntity in collection)
                    {
                        if (subEntity.ContainsKey(AddedRowKey) && subEntity[AddedRowKey] != null)
                        {
                            if ((bool)subEntity[AddedRowKey])
                            {
                                if (subEntity.ContainsKey(DeletedRowKey))
                                {
                                    object deleted = subEntity[DeletedRowKey];
            ...
            upload_fixes.ForEach(upload_fix =>
            {
                if (upload_fix != null && entity.ContainsKey(upload_fix.mc_nome_colonna) && entity[upload_fix.mc_nome_colonna] != null && upload_fix.UseRecordIDAsSubfolder)
                {
                        string __id = "";

                        if (entity.ContainsKey(GuidEntityKey))
                            __id = entity[GuidEntityKey].ToString();
                        else if (entity.ContainsKey("__id"))
            ...
            try
            {
                if (entity.ContainsKey(GuidEntityKey))
                {
                    string guidVal = entity[GuidEntityKey]?.ToString();
            ...
            if (!string.IsNullOrEmpty(metadata[0]._Metadati_Tabelle.md_after_save_server_method_name))
            {
                RawHelpers.InvokeUtilityHook(metadata[0]._Metadati_Tabelle.md_after_save_server_method_name, new object[] { userId, entity, dataMode.insert, result });
            }
        ...
        public static string RestoreflatData(Dictionary<string, object> entity, string route, string userId, MySqlConnection conn = null, MySqlTransaction trn = null)
        {
            ...
            try
            {
                ...
                if (conn == null)
                {
                    using (MySqlConnection connection = GetOpenConnection(isMeta, tab.md_conn_name))
                    {
                        return RawRestoreFlatData(entity, route, userId, ref query, metadata, connection, trn);
                    }
                }
                else
                {
                    return RawRestoreFlatData(entity, route, userId, ref query, metadata, conn, trn);
                }

            }
            catch (ValidationException e1)
            {
                Exception customEx = Utility.customizeException(RestoreFlatDataMethodName, e1, user.getUserByID(userId), route, query, entity);
                throw customEx;
            }
            catch (MySqlException e2)
            {
                Exception customEx = Utility.customizeException(RestoreFlatDataMethodName, e2, user.getUserByID(userId), route, query, entity);
                RawHelpers.logError(customEx, "restoreFlatData", query);
                throw customEx;
            }
            catch (Exception e3)
            {
                Exception customEx = Utility.customizeException(RestoreFlatDataMethodName, e3, user.getUserByID(userId), route, query, entity);
                RawHelpers.logError(customEx, "restoreFlatData", query);
                throw customEx;
        ...
        private static string RawRestoreFlatData(Dictionary<string, object> entity, string route, string userId, ref string query, List<_Metadati_Colonne> metadata, MySqlConnection connection, MySqlTransaction trn)
        {
            ...
            string ret = connection.Execute(query, commandTimeout: int.Parse(ConfigHelper.GetSettingAsString(AutoQueryTimeoutSetting)), transaction: trn).ToString();
            watch.Stop();
            RawHelpers.traceQuery(RestoreFlatDataMethodName, query, watch.ElapsedMilliseconds, route);
        ...
        private static string BuildDynamicRestoreQuery(Dictionary<string, object> entity, List<_Metadati_Colonne> metadata, string userId)
        {
            ...
            if (tabel.md_is_reticular)
            {
                ...
                string schemaPart = !string.IsNullOrEmpty(tabel.md_schema_name) ? "[" + tabel.md_schema_name + "]" : "";
                safetable_name = (string.IsNullOrEmpty(tabel.md_db_name) ? "" : "[" + tabel.md_db_name + "]." + schemaPart + ".") + EscapeDBObjectName(table_name);
            }
            ...
            metadata.ForEach((fld) =>
            {
                ...
                if (fld.mc_is_primary_key)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (!int.TryParse(entity[fld.mc_nome_colonna].ToString(), out ou) || fld.mc_db_column_type == ColTypeVarchar)
                                quote = "'";
            ...
            else if (!string.IsNullOrEmpty(ConfigHelper.GetSettingAsString(LogicDeleteFieldSetting)))
            {
                string logicRestoreField = ConfigHelper.GetSettingAsString(LogicDeleteFieldSetting);
                ...
                if (!string.IsNullOrEmpty(logicRestoreField))
                {
                    ...
                    {
                        throw new InvalidOperationException("Data can not be restored");
                    }
                }
            }
            else
            ...
            foreach (_Metadati_Colonne col in metadata)
            {
                if (tabel.md_parent_key_name == col.mc_nome_colonna)
                {
                    if (string.IsNullOrEmpty(parent_id))
                    {
                        if (sameLevel)
                        {
                            // Nodo fratello di una radice: nessun parent da impostare,
                            // la colonna parent resta assente dall'entity by-design.
                        }
                        else
                        {
        ...
        public static string GetCurrentFieldString(_Metadati_Tabelle tab, _Metadati_Colonne fld)
        {
            ...
            if (col != null)
            {
                // Lookup: il campo resta la colonna fisica (l'alias descrittivo e'
                // gestito altrove dal builder); nessuna trasformazione qui by-design.
            }
            ...
            if (fld.mc_db_column_type == ColTypePoint || fld.mc_ui_column_type == ColTypePoint)
            {
                current_fld = RawHelpers.sqlPointToString(current_fld, MySqlProviderName, fld);
            }
            else if (fld.mc_db_column_type == ColTypeGeometry || fld.mc_ui_column_type == ColTypeGeometry)
            {
        ...
        public static string BuildDynamicSelectQuery(List<_Metadati_Colonne> lst, List<SortInfo> SortInfo, List<GroupInfo> GroupInfo, PageInfo PageInfo, FilterInfos filterInfo, string logicOperator, bool hasServerOperation, MySqlConnection connection, out long totalRecords, List<AggregationInfo> aggregates, out List<AggregationResult> aggregateValues, string userId, string formulaLookup = "", int mcId = 0, string distinct = "", bool skipOrder = false, string extraFields = "", string parentRoute = "", SerializableDictionary<string, object> currentRecord = null)
        {
            ...
            using (metaRawModel mmd = new metaRawModel())
            {
                _Metadati_Tabelle tab = lst[0]._Metadati_Tabelle;
                _Metadati_Colonne pKey = lst.FirstOrDefault(x => x.mc_is_primary_key);
                string safetableName = RawHelpers.getStoreTableName(tab, MySqlProviderName);
                ...
                if (string.IsNullOrEmpty(distinct) && hasServerOperation)
                {
                    ...
                    if (!shadowing)
                    {
                        ...
                        try
                        {
                            ...
                            totalRecords = connection.QueryColumn<long>(countQry, commandTimeout: int.Parse(ConfigHelper.GetSettingAsString(AutoQueryTimeoutSetting))).FirstOrDefault();
                ...
                if (hasServerOperation && PageInfo.pageSize > 0)
                {
                    ...
                    if (string.IsNullOrEmpty(customSelectClause))
                    {
                        ...
                        {
                            finalQry = FinalizeServerSideGrouping(lst, mmd, GroupInfo, safetableName, join, where, connection, skiprecords, PageInfo);
                ...
                else
                {
                    ...
                    {
                        finalQry = FinalizeCientSideGrouping(lst, mmd, GroupInfo, safetableName, join, where);
        ...
        private static string ManageRelatedLookup(FilterInfos filterInfo, _Metadati_Tabelle tab, metaRawModel mmd, string logicOperator, string where, _Metadati_Colonne_Lookup lookuprelatedCol, _Metadati_Colonne pKey, string user_id)
        {
            ...
            if (lookuprelatedCol != null && !string.IsNullOrEmpty(lookuprelatedCol.mc_ui_lookup_default_filter))
            {
                ...
                foreach (string filterDef in filterDefs)
                {
                    ...
                    string currentFldLUp = RawHelpers.getFullyQualifiedColumnName(filteringCol, MySqlProviderName);
            ...
            if (pKey != null)
            {
                if (filterInfo != null && filterInfo.filters.FirstOrDefault(x => x.field == ExtraFilterFieldName) != null && lookuprelatedCol != null && string.IsNullOrEmpty(lookuprelatedCol.mc_ui_lookup_default_filter))
                {
                    // Filtro __extra gia' presente e nessun default-filter di lookup:
                    // la where resta invariata by-design (niente innerWhere da appendere).
                }
                else
                {
                    ...
                    if (innerWhere == "")
                        ...
                        innerWhereAppend = where == "" ? innerWhere : " AND (" + innerWhere.Substring(6) + ")";
        ...
        private static string JoinBuilder(_Metadati_Tabelle relatedTable, _Metadati_Colonne fld, _Metadati_Colonne_Lookup col, Dictionary<aliasPair, string> joins, string currentFld, _Metadati_Tabelle tab, string fieldList)
        {
            ...
            if (safeTextCol == null)
                throw new InvalidOperationException(string.Format("Colonna {0} Descrittiva per la lookup {1} mancante sulla tabella {2}", col.mc_ui_lookup_dataTextField, col.mc_nome_colonna, relatedTable.md_nome_tabella));
            ...
            if (isAlias && relatedTable.md_nome_tabella != ReticularTableName)
            {
            ...
            if (col.mc_ui_lookup_dataValueField != MetaColNomeColonna)
            {
                if (ap == null)
                {
        ...
        public static string BuildFinalJoin(_Metadati_Tabelle tab, Dictionary<aliasPair, string> joins, List<string> joinsAppend)
        {
            ...
            {
                StringBuilder joinListSb = new StringBuilder(CreateJoinString(joins, joinList));
                foreach (string jj in joinsAppend.Where(j => !string.IsNullOrWhiteSpace(j)))
                {
                    Match m = Regex.Match(jj.Trim(), ".+ AS (.+) ON ", RegexOptions.IgnoreCase);

                    if (m.Success && !joinListSb.ToString().ToUpper().Contains(" AS " + m.Groups[1].Value.ToUpper() + " ON ") /*|| !joinList.Contains(" AS " + m.Groups[1].Value + " on ") ||
                        !joinList.Contains(" as " + m.Groups[1].Value + " ON ") ||!joinList.Contains(" as " + m.Groups[1].Value + " on ")*/)
                    {
                        if (jj.Trim().StartsWith("AND "))
                            joinListSb.Append(string.Format(" {0} ", jj.Trim()));
                        else
                            joinListSb.Append(string.Format(" LEFT JOIN {0} ", jj.Trim()));
                    }
                }
                joinList = joinListSb.ToString();
            }
            else
            {
        ...
        private static void AppendFilterMultipleCheck(filterElement f, _Metadati_Colonne_Grid mm, _Metadati_Tabelle tabel, string realOperator, ref string where, string logicOperator, string userId)
        {
            ...
            using (MySqlConnection con = metaQueryMySql.GetOpenConnection(false, mmTable.md_conn_name))
            {
                ...
                string safeTableName = RawHelpers.getStoreTableName(mmTable, MySqlProviderName);
                string localTableName = RawHelpers.getStoreTableName(tabel, MySqlProviderName);
                ...
                if (realOperator == "eqor")
                {
                    ...
                    string part = " ( " + localTableName + "." + RawHelpers.escapeDBObjectName(mm.mc_ui_grid_local_id_field, MySqlProviderName) + " IN (" + nestedWhere + ") ";

                    where += ((where == "") ? " where " : " " + logicOperator) + part;

                }
                else if (realOperator == OpEqAll)
                {
                    ...
                    f.value.Split(',').ToList().ForEach((fltrVal) =>
                    {
                        ...
                        //NEEDED TO EXCLUDE THE COMBO-DESCRIPTION-FIELD ASSOCIATED WITH <mc_ui_grid_manytomany_local_id_field> LOOKUP-COLUMN
                        nestedWhere = string.Format("SELECT {1}.{0} FROM ", RawHelpers.escapeDBObjectName(mm.mc_ui_grid_manytomany_local_id_field, MySqlProviderName), safeTableName) + nestedWhere.Split(new string[] { "FROM" }, StringSplitOptions.None)[1];

                        complexNestedWhere = complexNestedWhere + (string.IsNullOrEmpty(complexNestedWhere) ? "" : " INTERSECT ") + nestedWhere;

                    });

                    where += ((where == "") ? " where " : " " + logicOperator) + " ( " + localTableName + "." + RawHelpers.escapeDBObjectName(mm.mc_ui_grid_local_id_field, MySqlProviderName) + " IN (" + complexNestedWhere + ") ";
                }

            }
        }

        private static string AppendFilter(_Metadati_Colonne fld, FilterInfos filterInfo, string logicOperator, string current_fld, string where, _Metadati_Tabelle tabel, string user_id = "", bool isNested = false)
        {
            filterInfo.filters.Where(x => (fld == null && x.nestedFilters != null) || (x.field != null && x.field.ToLower() == fld.mc_nome_colonna.ToLower() && x.field != ExtraFilterFieldName && !x.isHaving)).ToList().ForEach((f) =>
            {
                ...
                if (fld != null && fld.mc_ui_column_type == ColTypeMultipleCheck)
                {
                ...
                else if (realOperator == "eqorlogic" || realOperator == OpEqAll)
                {
                    string orWhere = "";
                ...
                else if (realOperator == "mapdistance")
                {
                    ...
                    if (mc.Count > 0)
                    {
                        ...
                        else if (fld.mc_ui_column_type == "google_map")
                        {
                            ...
                            _Metadati_Colonne pointField = tabel._Metadati_Colonnes.FirstOrDefault(x => x.mc_db_column_type == ColTypePoint);
                        ...
                        else if (mapProps != null && mapProps.map_type != ColTypePoint)
                            throw new NotSupportedException("Distance filter not supported from polyline and polygon.");
        ...
        private static void AppendBaseFilter(_Metadati_Colonne fld, string currentFld, string originalOperator, string realOperator, string filterValue, bool xtra, string logicOperator, ref string where, bool forceQuotes = false)
        {
            ...
            if (string.Equals(realOperator, "between", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (fld.mc_ui_column_type == ColTypeNumber || fld.mc_ui_column_type == ColTypeNumberSlider)
                {
            ...
            {
                if (originalOperator == OpContains)
                {
                    leftExtraOperator = quote + "%";
                    rightExtraOperator = "%" + quote;
                }
                if (originalOperator == OpStartsWith)
                {
                    leftExtraOperator = quote;
                    rightExtraOperator = "%" + quote;
                }
                if (originalOperator == OpEndsWith)
                {
                    leftExtraOperator = quote + "%";
                    rightExtraOperator = quote;
                }
            ...
            else if (fld.mc_ui_column_type == ColTypeNumber || fld.mc_ui_column_type == ColTypeNumberSlider)
            {
            ...
            else if (fld.mc_ui_column_type == ColTypeDatetime && filterValue != null && filterValue != "")
            {
                //FIX UTC TIME ISSUE 
                ...
                filterValue = d.ToString(SqlDateTimeFormat).Replace(".", ":");
            ...
            else if (fld.mc_ui_column_type == "date" && filterValue != null && filterValue != "")
            {
                ...
                filterValue = d.ToString(CompactDateFormat);
            ...
            else if ((fld.mc_ui_column_type == ColTypeBoolean || fld.mc_ui_column_type == ColTypeNumberBoolean || fld.mc_ui_column_type == "bit") && filterValue != null && filterValue != "")
            {
                if (filterValue.ToString().ToLower() == FalseString || filterValue.ToString().ToLower() == "0")
                    filterValue = "0";
                else
                    filterValue = "1";
        ...
        public static string BuildDynamicWhere(FilterInfos filterInfo, PageInfo PageInfo, metaRawModel mmd, List<_Metadati_Colonne> lst, _Metadati_Tabelle tab, _Metadati_Colonne pKey, string logicOperator, string distinct, Dictionary<aliasPair, string> joins, string formulaLookup, string userId, int mcId = 0, _Metadati_Colonne linkedCol = null) // 2026-04-30-build
        {
            ...
            if (tab.md_is_reticular)
            {
                ...
                string schemaPart = !string.IsNullOrEmpty(tab.md_schema_name) ? "`" + tab.md_schema_name + "`" : "";
                safetableName = (string.IsNullOrEmpty(tab.md_db_name) ? "" : "`" + tab.md_db_name + "`." + schemaPart + ".") + EscapeDBObjectName(tableName);
            ...
            if (!string.IsNullOrEmpty(ConfigHelper.GetSettingAsString(LogicDeleteFieldSetting)))
            {
                string logicDeleteField = ConfigHelper.GetSettingAsString(LogicDeleteFieldSetting);
            ...
            if (filterInfo != null && filterInfo.filters.Count > 0)
            {
                tab._Metadati_Colonnes.ForEach((fld) =>
                {
                    ...
                    if (filterInfo.filters.Any(x => x.field == ExtraFilterFieldName))
                        where = AppendFilter(fld, filterInfo, logicOperator, (currentFld), where, tab, userId);
                    else
                    {
                        ...
                        if (lc != null && lc.mc_ui_lookup_dataTextField == fld.mc_nome_colonna && !string.IsNullOrEmpty(formulaLookup))
                        {
                            ...
                            if (String.IsNullOrEmpty(formulaLookup))
                                filterField = !fld.mc_is_computed.HasValue || !fld.mc_is_computed.Value ? currentFld : fld.mc_nome_colonna;
                            else
                                filterField = useFormula ? formulaLookup : currentFld;

                            where = AppendFilter(fld, filterInfo, logicOperator, filterField, where, tab, userId);
                        }
                        else
                        {
                            string filterField;
            ...
            parseNestedFilters(filterInfo, logicOperator, tab, userId, ref where, ref addOperator);

            return where;
        }

        private static void parseNestedFilters(FilterInfos filterInfo, string logicOperator, _Metadati_Tabelle tab, string userId, ref string where, ref bool addOperator)
        {
            if (filterInfo.filters.Any(fi => fi.field != ExtraFilterFieldName))
            {
                bool hasNested = false;

                foreach (filterElement fi in filterInfo.filters.Where(z => z.field != ExtraFilterFieldName).ToList())
                {
                    ...
                    if (fi.nestedFilters != null && fi.nestedFilters.filters.Count > 0)
                    {
                        ...
                        if (where == "")
                            ...
                            whereOpening = !addOperator ? " (" : logicOperator + " ( ";
                        where += whereOpening;
                        int nestedIndex = 0;
        ...
        public static rawPagedResult GetManyToManyOptionsForInsert(string mc_ui_grid_route, int mc_ui_grid_pagesize, string mc_ui_grid_related_id_field, string mc_ui_grid_display_field, string user_id)
        {
            ...
            if (data != null)
            {
                ...
                {
                    rd.data[SelectedRowKey] = false;
                    rd.data[AddedRowKey] = false;
                    rd.data[DeletedRowKey] = false;
                }
            }
            return data;
        }
                ...
                using (MySqlConnection con = GetOpenConnection(false))
                {
                    foreach (_Metadati_Colonne_Slider chartCol in chartCols)
                    {
                        if (string.IsNullOrEmpty(chartCol.mc_chart_select))
                            throw new InvalidOperationException(string.Format("Chart select not specified for column {0}", chartCol.mc_display_string_in_view));
            ...
            if (relatedFullData != null)
            {
                foreach (Dapper.SqlMapper.FastExpando rd in relatedFullData.results)
                {
                    ...
                    if (selected != null)
                    {
                        ...
                        cloned[SelectedRowKey] = true;
                    }
                    else
                        cloned[SelectedRowKey] = false;

                    cloned[AddedRowKey] = false;
                    cloned[DeletedRowKey] = false;
        ...
        public static string BuildDynamicOrderBy(List<SortInfo> SortInfo, List<_Metadati_Colonne> lst, _Metadati_Tabelle tab, _Metadati_Colonne pKey, FilterInfos clonedfilters)
        {
            ...
            if (pKey != null)
            {
                ...
                if (clonedfilters.filters.FirstOrDefault(x => x.field == ExtraFilterFieldName) != null)
                {
                    ...
                    if (flr == null)
                    {
                        flr = clonedfilters.filters.First(x => x.field != ExtraFilterFieldName);
                ...
                {
                    if (sort == "")
                        fixOrder = string.Format(" ORDER BY {0}", pkOrder);
                    else
                        fixOrder = sort + (!string.IsNullOrEmpty(pkOrder) ? ", " + pkOrder : "");
                }
                else
        ...
        private static void AppendLoggingInsertFields(ref string field_list, ref string value_list, _Metadati_Tabelle tabel, string user_id, IDictionary<string, object> entity)
        {
            if (!string.IsNullOrEmpty(tabel.md_logging_insert_date_field_name))
            {
                ...
                {
                    StringBuilder fieldListSb = new StringBuilder(field_list);
                    StringBuilder valueListSb = new StringBuilder(value_list);
                    foreach (string fld in tabel.md_logging_insert_date_field_name.Split(','))
                    {
                        fieldListSb.Append((fieldListSb.Length == 0 ? "" : ", ") + fld);
                        valueListSb.Append((valueListSb.Length == 0 ? "" : ", ") + "'" + DateTime.Now.ToString(SqlDateTimeFormat).Replace(".", ":") + "'");
                        entity[fld] = DateTime.Now.ToString(CompactDateTimeFormat).Replace(".", ":");
                    }
                    field_list = fieldListSb.ToString();
                    value_list = valueListSb.ToString();
                }
                else
                {
                    field_list += (field_list == "" ? "" : ", ") + tabel.md_logging_insert_date_field_name;
                    value_list += (value_list == "" ? "" : ", ") + "'" + DateTime.Now.ToString(SqlDateTimeFormat).Replace(".", ":") + "'";
                    entity[tabel.md_logging_insert_date_field_name] = DateTime.Now.ToString(CompactDateTimeFormat).Replace(".", ":");
                }
            }
            ...
            if (!string.IsNullOrEmpty(tabel.md_logging_last_mod_date_field_name) && tabel.md_logging_last_mod_date_field_name != tabel.md_logging_insert_date_field_name)
            {
                ...
                else
                {
                    ...
                    value_list += (value_list == "" ? "" : ", ") + "'" + DateTime.Now.ToString(SqlDateTimeFormat).Replace(".", ":") + "'";
                    entity[tabel.md_logging_last_mod_date_field_name] = DateTime.Now.ToString(CompactDateTimeFormat).Replace(".", ":");
                }
            }
        ...
        private static void AppendLoggingUpdateFields(ref string field_value_list, _Metadati_Tabelle tabel, string user_id, Dictionary<string, object> entity)
        {
            if (!string.IsNullOrEmpty(tabel.md_logging_last_mod_date_field_name))
            {
                if (tabel.md_logging_last_mod_date_field_name.Contains(","))
                {
                    StringBuilder fieldValueListSb = new StringBuilder(field_value_list);
                    foreach (string fld in tabel.md_logging_last_mod_date_field_name.Split(','))
                    {
                        fieldValueListSb.Append((fieldValueListSb.Length == 0 ? "" : ", ") + fld + "=" + string.Format("'{0}'", DateTime.Now.ToString(SqlDateTimeFormat).Replace(".", ":")));
                        entity["fld"] = DateTime.Now.ToString(CompactDateTimeFormat).Replace(".", ":");
                    }
                    field_value_list = fieldValueListSb.ToString();
                }
                else
                {
                    field_value_list += (field_value_list == "" ? "" : ", ") + tabel.md_logging_last_mod_date_field_name + "=" + string.Format("'{0}'", DateTime.Now.ToString(SqlDateTimeFormat).Replace(".", ":"));
                    entity[tabel.md_logging_last_mod_date_field_name] = DateTime.Now.ToString(CompactDateTimeFormat).Replace(".", ":");
                }
        ...
        private static void AppendLoggingDeleteFields(ref string delete_log, _Metadati_Tabelle tabel, string user_id, Dictionary<string, object> entity)
        {
            ...
            {
                delete_log += (delete_log == "" ? "" : ", ") + tabel.md_loggingdelete_date_field_name + "=" + string.Format("'{0}'", DateTime.Now.ToString(SqlDateTimeFormat).Replace(".", ":"));
                entity[tabel.md_loggingdelete_date_field_name] = DateTime.Now.ToString(CompactDateTimeFormat).Replace(".", ":");
            }
        ...
        private static string FinalizeServerSideGrouping(List<_Metadati_Colonne> lst, metaRawModel mmd, List<GroupInfo> GroupInfo, string safetableName, string join, string where, MySqlConnection connection, int skiprecords, PageInfo PageInfo)
        {
            string fieldList = "";
        ...
        public static string BuildDynamicUpdateQuery(Dictionary<string, object> entity, List<_Metadati_Colonne> metadata, string user_id, bool importing = false, bool isMeta = false)
        {
            ...
            Dictionary<string, object> original = (importing || !entity.ContainsKey(OriginalEntityKey) ? new Dictionary<string, object>() : entity[OriginalEntityKey] as Dictionary<string, object>);
            ...
            if (table_name == "_metadati__colonne" && entity.ContainsKey(MetaColUiColumnType) && entity[MetaColUiColumnType] != null)
            {
                string widget = entity[MetaColUiColumnType].ToString();
                switch (widget)
                {
                    case ColTypeLookupById:
                        entity[MetaColVoaClass] = 2;

                        break;

                    case ColTypeNumberSlider:
                    case ColTypeNumber:
                        entity[MetaColVoaClass] = 3;

                        break;

                    case ColTypeUpload:
                        entity[MetaColVoaClass] = 5;

                        break;

                    case "button":
                        entity[MetaColVoaClass] = 6;

                        break;

                    case ColTypeMultipleCheck:
                        entity[MetaColVoaClass] = 4;

                        break;

                    case ColTypeHtmlArea:
                        entity[MetaColVoaClass] = 7;

                        break;

                    default:
                        entity[MetaColVoaClass] = 1;

                        break;
                }
            }

            string safetable_name = RawHelpers.getStoreTableName(tabel, MySqlProviderName);
            ...
            metadata.Where(x => !x.mc_is_computed.HasValue || !x.mc_is_computed.Value || x.GetType() == typeof(_Metadati_Colonne_Grid)).ToList().ForEach((fld) =>
            {
                ...
                {
                    if (fld.mc_nome_colonna == MetaColUiColumnType)
                    {
                ...
                if (tabel.md_logging_enable && (fld.mc_nome_colonna == tabel.md_logging_last_mod_date_field_name || fld.mc_nome_colonna == tabel.md_logging_last_mod_user_field_name))
                {
                    return;
                }
                ...
                if (!fld.mc_logic_editable.Value && !fld.mc_is_primary_key && fld.mc_nome_colonna != MetaColVoaClass)
                {
                    return;
                }
                ...
                if (fld.mc_validation_has.Value && fld.mc_validation_required.Value && valore == null && fld.mc_ui_column_type != ColTypeBoolean && fld.mc_ui_column_type != ColTypeNumberBoolean)
                    throw new ValidationException(string.Format("{0} non può essere null", fld.mc_display_string_in_view));

                valore = EscapeValue(valore);

                if (fld.mc_ui_column_type == ColTypeDatetime && valore != null && valore.ToString() != "")
                {
                    //FIX UTC TIME ISSUE 
                    string parsed = valore.ToString().Replace(@"""", "");
                    DateTime d = DateTime.Parse(parsed);
                    valore = d.ToString(SqlDateTimeFormat).Replace(".", ":");
                }
                else if (fld.mc_ui_column_type == "date" && valore != null && valore.ToString() != "")
                {
                    ...
                    {
                        DateTime d = DateTime.Parse(parsed);
                        valore = d.ToString(CompactDateFormat);
                    }
                ...
                else if (fld.mc_ui_column_type == ColTypeBoolean || fld.mc_ui_column_type == "bit" || fld.mc_db_column_type == "bit")
                {
                    ...
                    if (valore != null)
                    {
                        ...
                        else if (s == FalseString || s == "0" || s == "") valore = 0;
                    }
                    else
                    {
                ...
                else if (fld.mc_ui_column_type == ColTypeNumberBoolean)
                {
                    if (valore != null)
                    {
                        if (valore is bool)
                        ...
                        else
                        {
                            ...
                            else if (valore.ToString().ToLower() == FalseString)
                            {
                                valore = 0;
                            }
                ...
                else if (fld.mc_ui_column_type == ColTypeHtmlArea)
                {
                    if (valore != null)
                    {
                ...
                else if (fld.mc_db_column_type == ColTypePoint)
                {
                    if (valore != null && !string.IsNullOrEmpty(valore.ToString()))
                    {
                        ...
                        if (point == null)
                            point = RawHelpers.pointStringToPoint(rawGeo, MySqlProviderName);
                ...
                else if (fld.mc_db_column_type == "geography" && valore != null && !string.IsNullOrEmpty(valore.ToString()))
                {
                ...
                if (colGrid != null)
                {
                    ...
                    if (collection != null)
                    {
                        foreach (Dictionary<string, object> subEntity in collection)
                        {
                            ...
                            if (subEntity.ContainsKey(AddedRowKey) && (bool)subEntity[AddedRowKey])
                            {
                                if (subEntity.ContainsKey(DeletedRowKey))
                                {
                                    object deleted = subEntity[DeletedRowKey];
                            ...
                            else if (subEntity.ContainsKey(DeletedRowKey) && subEntity[DeletedRowKey] != null && (bool)subEntity[DeletedRowKey])
                            {
                                subEntity[colGrid.mc_ui_grid_manytomany_related_id_field] = subEntity[colGrid.mc_ui_grid_related_id_field];
                                subEntity[colGrid.mc_ui_grid_manytomany_local_id_field] = entity[colGrid.mc_ui_grid_local_id_field];
                ...
                {
                    if (valore.ToString() != "" && fld.mc_ui_is_password.HasValue && fld.mc_ui_is_password.Value && ConfigHelper.GetSettingAsString("IsPwdEncripted") == "true")
                    {
                    ...
                    if (valore.ToString() == "")
                        formattedValue = string.IsNullOrEmpty(fld.convert_null_to_string) ? "null" : "'" + valore.ToString() + "'";
                    else
                        formattedValue = valore.ToString();

                    field_value_list += (field_value_list == "" ? "" : ", ") + current_fld + "=" + string.Format("{0}{1}{0}", quote, formattedValue);

                    if (fld.mc_ui_column_type == ColTypeUpload)
                    {
        ...
        private static string BuildDynamicDeleteQuery(Dictionary<string, object> entity, List<_Metadati_Colonne> metadata, string user_id)
        {
            ...
            if (tabel.md_is_reticular)
            {
                table_name = ReticularTableName;
                string schemaPart = !string.IsNullOrEmpty(tabel.md_schema_name) ? "`" + tabel.md_schema_name + "`" : "";
                safetable_name = (string.IsNullOrEmpty(tabel.md_db_name) ? "" : "`" + tabel.md_db_name + "`." + schemaPart + ".") + RawHelpers.escapeDBObjectName(table_name, MySqlProviderName);
            }

            metadata.ForEach((fld) =>
            {
                string safecolumn_name = RawHelpers.escapeDBObjectName(RawHelpers.getStoreColumnName(fld), MySqlProviderName);
            ...
            else if (!string.IsNullOrEmpty(RawHelpers.ParseNull(ConfigHelper.GetSettingAsString(LogicDeleteFieldSetting))))
            {
                string logicDeleteField = ConfigHelper.GetSettingAsString(LogicDeleteFieldSetting);
                string logicDeleteValue = ConfigHelper.GetSettingAsString("logicDeleteValue");
        ...
        private static bool OptimisticCheck(Dictionary<string, object> entity, string route, List<_Metadati_Colonne> metadata)
        {
            ...
            using (MySqlConnection connection = metaQueryMySql.GetOpenConnection(isMeta, tab.md_conn_name))
            {
                ...
                foreach (string key in original.Keys)
                {
                    ...
                    if (string.Equals(col.mc_ui_column_type, ColTypeMultipleCheck, StringComparison.OrdinalIgnoreCase))
                    {
                    ...
                    if (col.mc_db_column_type != "varbinary" && col.mc_db_column_type != "binary" && (!col.mc_is_db_computed.HasValue || !col.mc_is_db_computed.Value) && (!col.mc_is_computed.HasValue || !col.mc_is_computed.Value) && col.mc_db_column_type != "float")
                    {
                        ...
                        string currentFld = RawHelpers.escapeDBObjectName(RawHelpers.getStoreColumnName(col), MySqlProviderName);
        ...
        private static HashSet<string> GetChangedFieldSet(Dictionary<string, object> entity)
        {
            ...
            if (entity == null || !entity.ContainsKey(ChangesEntityKey) || entity[ChangesEntityKey] == null)
                return result;

            if (entity[ChangesEntityKey] is IEnumerable<object> enumerableChanges)
            {
            ...
            HashSet<string> changedFields)
        {
            ...
            foreach (string key in original.Keys.Where(k => changedFields.Contains(k)))
            {
                keys.Add(key);
            }

            foreach (var pk in (metadata ?? new List<_Metadati_Colonne>()).Where(x => x.mc_is_primary_key && !string.IsNullOrEmpty(x?.mc_nome_colonna) && original.ContainsKey(x.mc_nome_colonna)))
            {
            ...
            _Metadati_Colonne_Grid manyToManyColumn)
        {
            ...
            string safeMmTable = RawHelpers.getStoreTableName(mmTable, MySqlProviderName);
            string localField = RawHelpers.escapeDBObjectName(manyToManyColumn.mc_ui_grid_manytomany_local_id_field, MySqlProviderName);
            string relatedField = RawHelpers.escapeDBObjectName(manyToManyColumn.mc_ui_grid_manytomany_related_id_field, MySqlProviderName);
        ...
        public static string BuildDynamicInsertQuery(IDictionary<string, object> entity, List<_Metadati_Colonne> metadata, string user_id, out string generated_pkey, bool importing = false, bool parametric = false)
        {
            ...
            bool base64Image = RawHelpers.ParseBool(ConfigHelper.GetSettingAsString("base64Image") ?? FalseString);
            ...
            metadata.Where(x => !x.mc_is_computed.HasValue || !x.mc_is_computed.Value).ToList().ForEach((fld) =>
            {
                ...
                if (tabel.md_logging_enable && (fld.mc_nome_colonna == tabel.md_logging_last_mod_date_field_name || fld.mc_nome_colonna == tabel.md_logging_last_mod_user_field_name || fld.mc_nome_colonna == tabel.md_logging_insert_date_field_name || fld.mc_nome_colonna == tabel.md_logging_insert_user_field_name))
                {
                    return;
                }
                ...
                if (!fld.mc_is_primary_key || string.IsNullOrEmpty(tabel.md_primary_key_type) || (fld.mc_logic_editable.HasValue && fld.mc_logic_editable.Value))
                {
                    ...
                    if (valore != null)
                    {
                        ...
                        if (!isPrimitiveType && (fld.mc_ui_column_type == ColTypeLookupById || fld.mc_ui_column_type == "dictionary"))
                        {
                            ...
                            {
                                valore = ((Dictionary<string, object>)valore)[ValueKey];
                            }
                        }
                    }
                    ...
                    if (!string.IsNullOrEmpty(fld.mc_default_value) && (valore == null || string.IsNullOrEmpty(valore.ToString())))
                    {
                        valore = fld.mc_default_value;
                    }

                    quote = ((fld.mc_db_column_type == "int" || fld.mc_db_column_type == "bit" || fld.mc_db_column_type == ColTypePoint || fld.mc_db_column_type == ColTypeGeometry || RawHelpers.ParseNull(valore) == "") ? "" : "'");

                    if (valore != null && !string.IsNullOrEmpty(valore.ToString()) && fld.mc_ui_is_password.Value && ConfigHelper.GetSettingAsString("IsPwdEncripted") == "true")
                    {
                        valore = Global.pbkdf2Hash(valore.ToString());
                    }

                    if (string.IsNullOrEmpty(RawHelpers.ParseNull(valore)) && (fld.mc_db_column_type == "date" || fld.mc_db_column_type == ColTypeDatetime))
                    {
                        quote = "";
                    }

                    if (valore != null && string.IsNullOrEmpty(quote) && string.IsNullOrEmpty(valore.ToString()))
                        valore = null;
                    ...
                    {
                        _Metadati_Colonne_Upload uploader = fld as _Metadati_Colonne_Upload;
                        if (uploader.isDBUpload && (entity.ContainsKey(GuidEntityKey) || entity.ContainsKey("__id")))
                        {
        ...
        public static string CloneData(IDictionary<string, object> entity, string route, string user_id, List<routePair> relatedRouteToClone)
        {
            ...
            using (MySqlConnection connection = GetOpenConnection(RawHelpers.checkIsMetaData(route), tab.md_conn_name))
            {
                ...
                {
                    ManageMaxKeyType(tab, pkeys[0], pkeys, entity, RawHelpers.getStoreColumnName(pkeys[0]), RawHelpers.getStoreTableName(tab, MySqlProviderName));
                }
                else
                {
        ...
        public static void ManageShadowCaching(dynamic caching, MySqlConnection con, metaRawModel mmd, _Metadati_Tabelle tab, List<_Metadati_Colonne> lst, string safetableName, string fieldList, string join, string where, string orderBy, int skiprecords, PageInfo PageInfo, List<SortInfo> SortInfo, FilterInfos filterInfo, List<AggregationInfo> aggregates, List<AggregationResult> aggregateValues, string logicOperator, string distinct, Dictionary<aliasPair, string> joins, string formulaLookup, string userId, int mcId, _Metadati_Colonne linkedCol, ref string finalQry, out long totalRecords)
        {
            ...
            if (string.IsNullOrEmpty(distinct))
            {
                ...
                try
                {
                    ...
                    totalRecords = con.QueryColumn<long>(countQry, commandTimeout: int.Parse(ConfigHelper.GetSettingAsString(AutoQueryTimeoutSetting))).FirstOrDefault();
        ...
        public static void traceQuery(string method, string query, long durata, string route)
        {
            ...
            bool trace = RawHelpers.ParseBool(ConfigHelper.GetSettingAsString("traceQuery") ?? FalseString);
            user user = RawHelpers.getUserFromCookie();
        ...
        public static void getUploadedFile(_Metadati_Tabelle tabel, string connectionString, _Metadati_Colonne pkey, _Metadati_Colonne_Upload uploader, string tabel_name, string __id, out byte[] file)
        {
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                ...
                string where = string.Format("{0} = {2}{1}{2}", RawHelpers.escapeDBObjectName(pkey.mc_nome_colonna, MySqlProviderName), __id, quote);
                string cmdStr = string.Format("select {0} from {1} where {2}", RawHelpers.escapeDBObjectName(uploader.MultipleUploadBlobFieldName, MySqlProviderName), tabel_name, where);
        ...
        public static void fixQueryReport(string user_id, dynamic report, string route, DbConnection connection, ref int needFilter, string[] filterSplit)
        {
            ...
            if (lst.Count > 0)
            {
                ...
                _Metadati_Tabelle tabel = lst[0]._Metadati_Tabelle;
                    ...
                    if (record.ContainsKey(key))
                    {
                        if (record[key] != null && !string.IsNullOrEmpty(record[key].ToString()))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    // commit_level tollerante: l'errore FK e' gia' loggato,
                                    // l'import prosegue con i record successivi by-design.
                                }
                            }
                        }
                        else
