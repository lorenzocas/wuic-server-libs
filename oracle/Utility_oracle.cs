...
namespace WEB_UI_CRAFTER.ProjectData.ServiziOracle
{
    public class Utility
    {
        ...
        private static bool InvokeHostCrudHook(string methodName, object[] args, Type[] parameterTypes = null)
        {
            ...
            typeCandidates.Add("WEB_UI_CRAFTER.ProjectData.ServiziOracle.UtilityHost");
            typeCandidates.Add("WEB_UI_CRAFTER.ProjectData.ServiziOracle.CustomCrudHooks");
        ...
        public bool switchUser(string token, string username, string password)
        {
            using (metaRawModel context = new metaRawModel())
            {
                ...
                if (infos != null)
                {
                    using (DbConnection connection = OracleProviderGateway.GetOpenConnection(true))
                    {
        ...
        {
            var utente = OracleProviderGateway.getUserByID(user_id);
