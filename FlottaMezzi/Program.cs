...
internal static class Program
{
    ...
    private static IHostBuilder CreateHostBuilder(string[] args, string hostProjectRoot, string legacyRoot) =>
        Host.CreateDefaultBuilder(args)            .ConfigureAppConfiguration((hostingContext, config) =>
            ...
            .ConfigureWebHostDefaults(webBuilder =>
            {
                ...
                //   - When the same binary is launched standalone (`dotnet FlottaMezzi.dll`),
    ...
    private static string ResolveLegacyRoot(string hostProjectRoot)
    {
        ...
        if (string.IsNullOrWhiteSpace(configured))
        {
            ...
            configured = bootstrapConfig["FlottaMezzi:StaticAngularRoot"] ?? bootstrapConfig["NugetHost:StaticAngularRoot"];
        }
    ...
    private static string ResolveHostProjectRoot()
    {
        ...
        if (File.Exists(Path.Combine(devCandidate, "FlottaMezzi.csproj")))
            return devCandidate;
