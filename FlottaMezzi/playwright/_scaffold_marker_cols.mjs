...
// scaffoldTable senza createMenu re-scaffolda solo le colonne mancanti senza duplicare la table
const t1 = await call('scaffolding.scaffoldTable', { ...SHARED, table: 'mezzi', createMenu: false, parentMenuId: 0, schema: 'dbo' });
console.log('mezzi re-scaffold:', t1.status, JSON.stringify(t1.json).slice(0, 300));

// Re-scaffold view per pickup colonne nuove (colore_marker, svg_marker)
const v1 = await call('scaffolding.scaffoldView', { ...SHARED, view: 'vw_mezzi_posizioni_giorno', createMenu: false, parentMenuId: 0 });
console.log('view refresh:', v1.status, JSON.stringify(v1.json).slice(0, 200));
